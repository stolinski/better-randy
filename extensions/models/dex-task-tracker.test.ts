...
class FakeDexCommandAdapter implements DexTaskCommandAdapter {
	...
	#executeCommand(args: readonly string[]): DexCommandResult {
		return args[0] === 'show' ? this.#showTask(args[1]) : this.#mutateTask(args);
	}
	...
	#mutateTask(args: readonly string[]): DexCommandResult {
		if (this.failMutation) return { code: 9, stdout: '', stderr: 'fixture command failure' };
		if (this.task === null) return { code: 1, stdout: '', stderr: 'Task not found' };
...
Deno.test('model exposes the locked type, version, resources, and method set', () => {
	...
	assert.deepEqual(
		DexTaskGetArgsSchema.parse({ taskId: 'task-123', ownerToken: 'evaluated-global' }),
		{ taskId: 'task-123' }
	);
});
...
Deno.test(
	...
	async () => {
		...
		await executeDexTaskGet({ taskId: 'task-123' }, fixture.context, dependencies(adapter));

		assert.deepEqual(adapter.cliCalls[0], {
			cwd: '/fixture/repository',
			args: ['show', 'task-123', '--json']
		});
		const snapshot = DexTaskSnapshotSchema.parse(taskWrites(fixture.writes)[0].data);
...
Deno.test('start rejects duplicate starts before invoking a mutation', async () => {
	const adapter = new FakeDexCommandAdapter(rawDexTask({ started_at: FIXED_NOW }));
	const fixture = fixtureContext();
	await assertTrackerFailure(
		executeDexTaskStart({ taskId: 'task-123' }, fixture.context, dependencies(adapter)),
...
Deno.test('complete rejects an already completed task', async () => {
	const adapter = new FakeDexCommandAdapter(
		rawDexTask({ completed: true, started_at: FIXED_NOW, completed_at: FIXED_NOW })
	);
	const fixture = fixtureContext();
	await assertTrackerFailure(
		executeDexTaskComplete(
...
Deno.test('complete uses the exact structured result and commit argument vector', async () => {
	...
	await executeDexTaskComplete(
		{
			taskId: 'task-123',
			result,
			commit: { kind: 'commit', sha: '0123456789abcdef0123456789abcdef01234567' }
		},
		fixture.context,
		dependencies(adapter)
	);
	...
	assert.equal(DexTaskSnapshotSchema.parse(taskWrites(fixture.writes)[0].data).completed, true);
});
...
Deno.test(
	...
	async () => {
		...
		const fixture = fixtureContext();
		await executeDexTaskReopen({ taskId: 'task-123' }, fixture.context, dependencies(adapter));
...
Deno.test('reopen rejects an incomplete task before calling MCP', async () => {
	...
	const fixture = fixtureContext();
	await assertTrackerFailure(
		executeDexTaskReopen({ taskId: 'task-123' }, fixture.context, dependencies(adapter)),
		'task-not-completed'
	);
...
Deno.test('malformed Dex JSON persists invalid-json and throws', async () => {
	...
	const fixture = fixtureContext();
	await assertTrackerFailure(
		executeDexTaskGet({ taskId: 'task-123' }, fixture.context, dependencies(adapter)),
		'invalid-json'
	);
	const receipt = DexTaskTrackerReceiptSchema.parse(receiptWrites(fixture.writes)[0].data);
...
Deno.test('nonzero Dex mutation persists command-failed without stderr', async () => {
	...
	assert.equal(JSON.stringify(receipt).includes('fixture command failure'), false);
});
...
Deno.test('same repository and task invocations are serialized in-process', async () => {
	...
	await Promise.all([
		executeDexTaskGet({ taskId: 'task-123' }, first.context, dependencies(adapter)),
		executeDexTaskGet({ taskId: 'task-123' }, second.context, dependencies(adapter))
	]);
