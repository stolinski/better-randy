...
export const SupersAgentWorktreePurposeSchema = z.enum([
  "sentry-reproduction",
  "delivery-coding",
]);
export type SupersAgentWorktreePurpose = z.infer<
  typeof SupersAgentWorktreePurposeSchema
>;
...
export const PrepareSupersAgentWorktreeArgsSchema = z
  ...
  .strict();
export type PrepareSupersAgentWorktreeArgs = z.infer<
  typeof PrepareSupersAgentWorktreeArgsSchema
>;
...
export const SupersAgentWorktreeBindingSchema = z
  ...
  .strict();
export type SupersAgentWorktreeBinding = z.infer<
  typeof SupersAgentWorktreeBindingSchema
>;

export const SupersAgentWorktreeIntentSchema = SupersAgentWorktreeBindingSchema
  .extend({
    preparedAt: z.string().datetime(),
  }).strict();
export type SupersAgentWorktreeIntent = z.infer<
  typeof SupersAgentWorktreeIntentSchema
>;

export const SupersAgentWorktreeClaimSchema = SupersAgentWorktreeIntentSchema
  .extend({
    headSha: z.string().regex(SHA40_PATTERN),
    stateHash: z.string().regex(SHA64_PATTERN),
  }).strict();
export type SupersAgentWorktreeClaim = z.infer<
  typeof SupersAgentWorktreeClaimSchema
>;
...
export const SupersAgentIntegrationIntentSchema = z
  ...
  .strict();
export type SupersAgentIntegrationIntent = z.infer<
  typeof SupersAgentIntegrationIntentSchema
>;
...
const SupersAgentWorktreeRemovalAuthorizationSchema = z.discriminatedUnion(
  "kind",
  [
...
export const RemoveSupersAgentWorktreeArgsSchema = z
  ...
  .strict();
export type RemoveSupersAgentWorktreeArgs = z.infer<
  typeof RemoveSupersAgentWorktreeArgsSchema
>;
...
export const SupersAgentWorktreeRemovalIntentSchema = z
  .object({
    ...
    worktreePath: z.string().min(1),
    attachedBranch: z.string().min(1),
    authorizationKind: z.enum(["unchanged", "committed", "superseded"])
      .optional(),
...
export const SupersAgentWorktreeReconciliationReceiptSchema = z
  .object({
    ...
    repositoryDir: z.string().min(1),
    centralRevision: z.string().regex(SHA40_PATTERN),
    items: z.array(SupersAgentWorktreeReconciliationItemSchema).max(
...
async function requiredGitBytes(
  ...
  if (result.stdout.length > maxBytes) {
    throw new Error(
      `git ${args[0]} output exceeds the ${maxBytes}-byte safety limit`,
    );
  }
...
async function requiredGitOutput(
  ...
  try {
    return strictDecoder.decode(
      await requiredGitBytes(deps, cwd, args, maxBytes),
    );
  } catch (error) {
    throw new Error(
      `git ${args[0]} returned invalid UTF-8 for a textual protocol`,
      { cause: error },
    );
  }
}
...
export async function createSupersAgentStableIdentityHash(
...
export async function createSupersAgentRepositoryStateHash(
  ...
  for (const relativePath of untracked.split("\0").filter(Boolean).sort()) {
    ...
    if (info.isSymlink) {
      throw new Error(
        `untracked symlinks cannot be repository evidence: ${relativePath}`,
      );
    }
    if (!info.isFile) {
      throw new Error(
        `untracked non-regular files cannot be repository evidence: ${relativePath}`,
      );
    }
    ...
    if (info.size > MAX_UNTRACKED_FILE_BYTES) {
      throw new Error(
        `untracked file exceeds the repository evidence size limit: ${relativePath}`,
      );
    }
    aggregateBytes += info.size;
    if (aggregateBytes > MAX_UNTRACKED_AGGREGATE_BYTES) {
      throw new Error(
        "untracked files exceed the aggregate repository evidence size limit",
      );
    }
    const canonicalPath = await deps.realPath(absolutePath);
    if (!isContainedPath(canonicalRoot, canonicalPath)) {
      throw new Error(
        `untracked file resolves outside the repository: ${relativePath}`,
      );
    }
    validatedFiles.push({
      relativePath,
  ...
  const parts: Uint8Array[] = [
    encoder.encode("tracked-diff-v1"),
    encoder.encode(diff),
  ];
...
async function requireStableCentralRepository(
  ...
  if (topLevel !== canonicalDir) {
    throw new Error(
      "central repository path is not its canonical Git top-level",
    );
  }
  const gitDir = (
    await requiredGitOutput(
  ...
  if (gitDir !== commonDir) {
    throw new Error(
      "central repository must be the primary worktree, not a linked worktree",
    );
  }
  const firstHead = (
    await requiredGitOutput(
  ...
  if (secondHead !== firstHead) {
    throw new Error(
      "central repository HEAD changed during worktree preflight",
    );
  }
...
async function createBinding(
  ...
  if (
    worktreeRoot === repositoryDir ||
    isContainedPath(repositoryDir, worktreeRoot)
  ) {
    throw new Error(
      "agent worktree root must be outside the central repository",
    );
  }
...
function bindingMatches(
  ...
  return expectedBindings.some((expected) =>
  ...
  const intentMatches = expectedBindings.some(
    (binding) =>
      JSON.stringify(claimIntent) ===
        JSON.stringify(createIntent(binding, claim.preparedAt)),
  );
  if (!intentMatches) {
    throw new Error(
      "worktree claim content does not match its content-addressed identity",
    );
  }
...
type SupersAgentInvocationState =
  SupersAgentWorktreeReconciliationItem["invocationState"];
...
function sumWorktreeLogicalBytes(
  items: readonly SupersAgentWorktreeReconciliationItem[],
): number {
  let total = 0;
  for (const item of items) {
    ...
    if (!Number.isSafeInteger(total)) {
      throw new Error(
        "reconciled worktree sizes exceed the safe integer range",
      );
    }
  }
  return total;
...
async function readBoundFinalInvocation(
  ...
  if (invocation.invocationId !== claim.invocationId) {
    throw new Error(
      "CLI-agent invocation identity does not match the worktree claim",
    );
  }
  ...
  if (
    ...
    throw new Error(
      "CLI-agent launch claim does not match the exact worktree expectation",
    );
  }
  for (const [key, value] of Object.entries(claim.expectedInvocationTags)) {
...
async function observeClaimedWorktree(
  ...
  const matchingRecords = (await listWorktrees(deps, claim.repositoryDir))
    .filter(
      (candidate) =>
  ...
  if (matchingRecords.length === 0) {
    if (await deps.pathExists(claim.worktreePath)) {
      throw new Error(
        "claimed worktree path exists without its exact Git registration",
      );
    }
    return null;
  }
  if (matchingRecords.length !== 1) {
    throw new Error(
      "claimed worktree path and branch resolve to conflicting Git registrations",
    );
  }
  ...
  if (
    ...
    throw new Error(
      "claimed worktree Git registration conflicts with its exact identity",
    );
  }
  if (!(await deps.pathExists(claim.worktreePath))) {
...
async function verifySupersededWorktreeCleanup(
  ...
  const receiptName =
    `supers-agent-worktree-superseded-${claim.claimId}-${centralRevision}`;
  ...
  if (existing !== null) {
    ...
    if (
      ...
      existing.fingerprint !==
        (await createSupersAgentStableIdentityHash(existingBase))
    ) {
      throw new Error(
        "superseded cleanup receipt conflicts with the exact worktree claim",
      );
    }
    return existing;
  }
  ...
  const claimAgeMs = Math.max(
    0,
    deps.now().getTime() - Date.parse(claim.preparedAt),
  );
  if (invocation === null && claimAgeMs < ABANDONED_WORKTREE_MIN_AGE_MS) {
    throw new Error(
      "superseded cleanup preserves a recent worktree without final invocation",
    );
  }
  const observed = await observeClaimedWorktree(deps, claim);
  if (observed === null) {
    throw new Error(
      "superseded cleanup found no worktree or stale registration",
    );
  }
  if (observed.dirty) {
...
async function requireNoPreparationCollision(
  ...
  const registration = (await listWorktrees(deps, binding.repositoryDir)).find(
    (record) =>
      ...
      record.attachedBranch === binding.attachedBranch,
  );
...
async function requireExactWorktree(
  ...
  if (record === undefined) {
    throw new Error(
      "existing path is not a registered worktree of the central repository",
    );
  }
  if (
    record.headSha !== claim.baseRevision ||
    record.attachedBranch !== claim.attachedBranch
  ) {
    throw new Error(
      "registered worktree conflicts with the prepared branch or revision",
    );
  }
  const topLevel = (
    await requiredGitOutput(
  ...
  if (headSha !== claim.baseRevision) {
    throw new Error(
      `worktree HEAD mismatch: expected ${claim.baseRevision}, got ${headSha}`,
    );
  }
  const stateHash = await createSupersAgentRepositoryStateHash(
    claim.worktreePath,
    deps,
  );
  if (expectedStateHash !== undefined && stateHash !== expectedStateHash) {
    throw new Error(
      "worktree repository state changed after the agent invocation",
    );
  }
...
async function requireCommittedCheckoutIdentity(
  ...
  if (
    ...
    throw new Error(
      "registered worktree changed after committed evidence collection",
    );
  }
...
async function collectCommittedWorktreeEvidence(
  ...
  if (
    ...
    throw new Error(
      "registered worktree conflicts with the committed branch or revision",
    );
  }
  const topLevel = (
    await requiredGitOutput(
  ...
  if (
    ...
    throw new Error(
      "committed worktree checkout identity does not match its claim",
    );
  }
  ...
  if (!ancestor.success) {
    if (ancestor.code === 1) {
      throw new Error("worktree base is not an ancestor of its commit");
    }
    throw gitFailure(
      [
        "merge-base",
  ...
  if (fields.length / 2 > MAX_CHANGED_PATHS) {
    throw new Error(
      `committed worktree exceeds the ${MAX_CHANGED_PATHS}-path safety limit`,
    );
  }
  ...
  for (let index = 0; index < fields.length; index += 2) {
    ...
    if (blobSize > MAX_CHANGED_BLOB_BYTES) {
      throw new Error(
        `changed path exceeds the ${MAX_CHANGED_BLOB_BYTES}-byte safety limit: ${path}`,
      );
    }
  ...
  return {
    ...
    changedPaths,
    changedPathsDigest: await createSupersAgentStableIdentityHash({
      changedPaths,
    }),
...
async function readExternalParsedResource<T>(
  ...
  if (content === null) {
    throw new Error(`external integration resource is missing: ${name}`);
  }
  try {
    return schema.parse(JSON.parse(strictDecoder.decode(content)));
  } catch (error) {
    throw new Error(`external integration resource is malformed: ${name}`, {
      cause: error,
    });
  }
}
...
export interface SupersAgentWorktreeOperations {
  ...
  verifySupersAgentWorktreeUnchanged(
    ...
    SupersAgentWorktreeResourceResult<SupersAgentWorktreeUnchangedReceipt>
  >;
  verifySupersAgentWorktreeCommit(
    args: VerifySupersAgentWorktreeCommitArgs,
    context: SupersAgentWorktreeMethodContext,
  ): Promise<
    SupersAgentWorktreeResourceResult<SupersAgentWorktreeCommitReceipt>
  >;
  verifySupersAgentIntegration(
    args: VerifySupersAgentIntegrationArgs,
    context: SupersAgentWorktreeMethodContext,
  ): Promise<
    SupersAgentWorktreeResourceResult<SupersFactoryIntegrationReceipt>
  >;
  removeSupersAgentWorktree(
    args: RemoveSupersAgentWorktreeArgs,
    context: SupersAgentWorktreeMethodContext,
  ): Promise<
    SupersAgentWorktreeResourceResult<SupersAgentWorktreeRemovalReceipt>
  >;
  reconcileSupersAgentWorktrees(
    args: ReconcileSupersAgentWorktreesArgs,
    context: SupersAgentWorktreeMethodContext,
  ): Promise<
    SupersAgentWorktreeResourceResult<SupersAgentWorktreeReconciliationReceipt>
  >;
}
  ...
  const operations: SupersAgentWorktreeOperations = {
    async prepareSupersAgentWorktree(argsInput, context) {
      ...
      if (
        ...
        !bindingMatches(existingBinding, supportedBindings)
      ) {
        throw new Error(
      ...
      if (intent === null) {
        creationSnapshot = await requireCentralContainsRevision(
          deps,
          canonicalRepositoryDir,
          args.baseRevision,
        );
      ...
      if (existingClaim !== null) {
        ...
        await requireExactWorktree(
          deps,
          existingClaim,
          existingClaim.stateHash,
        );
        return { resource: existingClaim, dataHandles: createdHandles };
      }

      if (
        existingIntent !== null && (await deps.pathExists(intent.worktreePath))
      ) {
        const recovered = await requireExactWorktree(deps, intent);
        const recoveredClaim: SupersAgentWorktreeClaim =
          SupersAgentWorktreeClaimSchema.parse({
            ...intent,
      ...
      const existingRecord = (await listWorktrees(deps, intent.repositoryDir))
        .find(
      ...
      if (
        ...
        (pathAlreadyExists || branchAlreadyExists)
      ) {
        throw new Error(
          "deterministic worktree path or branch already exists with no recoverable worktree",
        );
      ...
      if (existingRecord === undefined) {
        ...
        if (!addResult.success) {
          const recovered = (await listWorktrees(deps, intent.repositoryDir))
            .find(
              (record) => record.path === intent.worktreePath,
            );
      ...
      const claim: SupersAgentWorktreeClaim = SupersAgentWorktreeClaimSchema
        .parse({
          ...intent,
    ...
    async verifySupersAgentWorktreeUnchanged(argsInput, context) {
      const args = VerifySupersAgentWorktreeUnchangedArgsSchema.parse(
        argsInput,
      );
      ...
      if (invocation === null) {
        throw new Error(
          `CLI-agent invocation resource is missing: invocation-${claim.invocationId}`,
        );
      }
      if (
        !invocation.success || invocation.exitCode !== 0 || invocation.timedOut
      ) {
        throw new Error("CLI-agent invocation did not finish successfully");
      }
      await requireExactWorktree(deps, claim, claim.stateHash);
      const receipt = expectedUnchangedReceipt(
        claim,
        receiptId,
    ...
    async verifySupersAgentWorktreeCommit(argsInput, context) {
      ...
      if (args.invocationResourceName !== `invocation-${args.invocationId}`) {
        throw new Error(
          "CLI-agent invocation resource name is not caller-owned",
        );
      }
      const claim = await readParsedResource(
        context,
      ...
      if (
        ...
        throw new Error(
          "commit verification arguments do not match the exact worktree claim",
        );
      }
      ...
      const invocationRaw = await context.readResource(
        args.invocationResourceName,
      );
      if (invocationRaw === null) {
        throw new Error(
          `CLI-agent invocation resource is missing: ${args.invocationResourceName}`,
        );
      }
      ...
      if (
        ...
        throw new Error(
          "CLI-agent invocation does not match commit verification authority",
        );
      }
      for (const [key, value] of Object.entries(claim.expectedInvocationTags)) {
      ...
      const launchRaw = await context.readResource(
        `launch-claim-${args.invocationId}`,
      );
      if (launchRaw === null) {
        throw new Error(
          `CLI-agent launch claim is missing: launch-claim-${args.invocationId}`,
        );
      }
      ...
      if (
        ...
        throw new Error(
          "CLI-agent launch claim does not match commit verification authority",
        );
      }
      for (const [key, value] of Object.entries(claim.expectedInvocationTags)) {
        if (launch.tags[key] !== value) {
      ...
      const evidence = await collectCommittedWorktreeEvidence(
        deps,
        claim,
        args,
      );
      ...
      const finalEvidence = await collectCommittedWorktreeEvidence(
        deps,
        claim,
        args,
      );
      if (JSON.stringify(finalEvidence) !== JSON.stringify(evidence)) {
        throw new Error(
          "committed worktree evidence changed during verification",
        );
      }
      ...
      receipt.fingerprint = await createSupersAgentStableIdentityHash(
        receiptBase,
      );
      await requireCommittedCheckoutIdentity(
        deps,
        claim,
        args.expectedCommitRevision,
      );
    ...
    async verifySupersAgentIntegration(argsInput, context) {
      ...
      if (commitReceipt === null) {
        throw new Error("integration commit receipt is missing");
      }
      const recomputedCommitFingerprint =
        await createSupersAgentStableIdentityHash(
          withoutFingerprint(
            commitReceipt as unknown as Record<string, unknown>,
          ),
        );
      if (
      ...
      if (
        ...
        throw new Error(
          "official Git cherry-pick receipt does not bind the exact child commit",
        );
      }
      const cherryPickDigest = await createSupersAgentStableIdentityHash(
        cherryPick,
      );
      const resourceSuffix =
        `${commitReceipt.workItem}-${args.expectedPostRevision}`;
      ...
      proposedIntent.fingerprint = await createSupersAgentStableIdentityHash(
        withoutFingerprint(
          proposedIntent as unknown as Record<string, unknown>,
        ),
      );
      const existingIntent = await readParsedResource(
        context,
      ...
      if (existingIntent !== null) {
        ...
        expectedExisting.fingerprint =
          await createSupersAgentStableIdentityHash(
            withoutFingerprint(
              expectedExisting as unknown as Record<string, unknown>,
            ),
          );
        requireExactResource(
          SupersAgentIntegrationIntentSchema,
      ...
      if (parentFields.length !== 2 || !SHA40_PATTERN.test(parentFields[1])) {
        throw new Error(
          "integrated revision is not one exact single-parent serialized result",
        );
      }
      ...
      const fields = nameStatus.split("\0").filter(Boolean);
      if (
        fields.length === 0 || fields.length % 2 !== 0 ||
        fields.length / 2 > MAX_CHANGED_PATHS
      ) {
        throw new Error(
          "integrated revision has invalid changed-path evidence",
        );
      }
      const changedPaths: string[] = [];
      for (let index = 0; index < fields.length; index += 2) {
        if (fields[index] !== "A" && fields[index] !== "M") {
          throw new Error(
            `unsupported integrated path status: ${fields[index]}`,
          );
        }
      ...
      if (
        JSON.stringify(changedPaths) !==
          JSON.stringify(commitReceipt.changedPaths) ||
        (await createSupersAgentStableIdentityHash({ changedPaths })) !==
          commitReceipt.changedPathsDigest
      ) {
        throw new Error(
          "integrated changed paths differ from the verified child receipt",
        );
      }
      ...
      if (patchDigest !== commitReceipt.diffDigest) {
        throw new Error(
          "integrated patch digest differs from the verified child receipt",
        );
      }
      await requireStableCentralRevision(
        deps,
      ...
      const receiptId = await createSupersDeterministicContractHash(
        receiptBase,
      );
      ...
      if (existingReceipt !== null && existingManifest === null) {
        throw new Error(
          "integration receipt exists without its handoff manifest",
        );
      }
    ...
    async removeSupersAgentWorktree(argsInput, context) {
      ...
      let committedAuthorization:
        | {
          receipt: SupersAgentWorktreeCommitReceipt;
          verifyArgs: VerifySupersAgentWorktreeCommitArgs;
        }
        | null = null;
      let supersededAuthorization: SupersAgentSupersededWorktreeReceipt | null =
        null;
      if (authorization.kind === "unchanged") {
        ...
        if (unchanged === null) {
          throw new Error(
            "an exact unchanged receipt is required before worktree removal",
          );
        }
        requireExactResource(
          SupersAgentWorktreeUnchangedReceiptSchema,
        ...
        if (
          authorization.receiptName !==
            `supers-agent-worktree-commit-${authorization.receiptId}`
        ) {
          throw new Error(
            "committed cleanup authorization resource name mismatch",
          );
        }
        ...
        if (committed === null) {
          throw new Error(
            "exact committed worktree receipt is required before removal",
          );
        }
        ...
        if (
          ...
          committed.fingerprint !==
            (await createSupersAgentStableIdentityHash(committedBase)) ||
          ...
          throw new Error(
            "committed cleanup authorization conflicts with the exact worktree claim",
          );
        }
      ...
      const quarantinePath = `${
        ownedRoot === "/" ? "" : ownedRoot
      }/.supers-agent-quarantine-${receiptId}`;
      ...
      const matchingRegistration =
        (await listWorktrees(deps, claim.repositoryDir)).find(
          (candidate) =>
            candidate.path === claim.worktreePath ||
            candidate.attachedBranch === claim.attachedBranch,
        );
      if (
        existingIntent === null && !originalExists &&
        supersededAuthorization === null
      ) {
        throw new Error(
          "cannot begin worktree removal because the claimed worktree is absent",
        );
      }
      if (existingIntent === null && quarantineExists) {
        throw new Error(
          "cannot begin worktree removal from an ambiguous quarantine state",
        );
      }
      if (
        !originalExists &&
        ...
        const registrationMatchesQuarantine = quarantineExists &&
          matchingRegistration.path === quarantinePath &&
          matchingRegistration.attachedBranch === claim.attachedBranch &&
          matchingRegistration.headSha === expectedSupersededHead;
        if (!registrationMatchesOriginal && !registrationMatchesQuarantine) {
          throw new Error(
            "stale worktree registration conflicts with superseded cleanup evidence",
          );
        }
      }
      if (originalExists && !quarantineExists) {
        if (committedAuthorization !== null) {
          ...
          if (
            ...
            JSON.stringify(observed.changedPaths) !==
              JSON.stringify(committed.changedPaths)
          ) {
            throw new Error(
              "committed worktree no longer matches its cleanup authorization",
            );
          }
          ...
          if (
            ...
            throw new Error(
              "worktree changed after superseded cleanup verification",
            );
          }
          await requireStableCentralRepository(
            deps,
      ...
      const removalAlreadyApplied = existingIntent !== null &&
        !originalExists &&
        !quarantineExists &&
        (supersededAuthorization === null ||
          matchingRegistration === undefined);
      if (!removalAlreadyApplied) {
        if (!originalExists && !quarantineExists) {
          ...
          if (!(await deps.pathExists(quarantinePath))) {
            throw new Error(
              "exact worktree quarantine is absent after atomic rename",
            );
          }
          const repairArgs = ["worktree", "repair", quarantinePath] as const;
          const repairResult = await deps.runGit(
            claim.repositoryDir,
            repairArgs,
            {
              stdoutBytes: MAX_GIT_PROTOCOL_BYTES,
          ...
          const expectedHead = committedAuthorization?.receipt.commitRevision ??
          ...
          if (
            ...
            throw new Error(
              "Git registration does not match the exact quarantined worktree",
            );
          }
          const removeArgs = [
            "worktree",
          ...
          const removeResult = await deps.runGit(
            claim.repositoryDir,
            removeArgs,
            {
              stdoutBytes: MAX_GIT_PROTOCOL_BYTES,
          ...
          if (
            !removeResult.success && (await deps.pathExists(quarantinePath))
          ) {
      ...
      if (await deps.pathExists(quarantinePath)) {
        throw new Error(
          "quarantined worktree still exists after exact removal",
        );
      }
    ...
    async reconcileSupersAgentWorktrees(argsInput, context) {
      ...
      for (const claimId of [...args.claimIds].sort()) {
        ...
        if (claim === null) {
          throw new Error(`worktree claim is missing: ${claimId}`);
        }
        await requireValidClaimIdentity(claim, deps);
        const observed = await observeClaimedWorktree(deps, claim);
        const measurement = observed?.presence === "present"
          ? await deps.measureDirectory(claim.worktreePath)
          : { logicalBytes: 0, complete: true };
        const invocation = observed === null
          ? null
          : await readBoundFinalInvocation(context, claim);
        const invocationState = getSupersAgentInvocationState(invocation);
        const ageMs = Math.max(
          0,
          reconciledAtMs - Date.parse(claim.preparedAt),
        );
        const itemBase = {
          claimId: claim.claimId,
          workItem: claim.workItem,
      ...
      const receiptCore = {
        ...
        items,
        removedCount:
          items.filter((item) => item.disposition.startsWith("removed-"))
            .length,
        preservedCount:
          items.filter((item) => item.disposition.startsWith("preserved-"))
            .length,
        absentCount:
          items.filter((item) => item.disposition === "absent").length,
        logicalBytesBefore: sumWorktreeLogicalBytes(items),
        reconciledAt,
      };
      const reconciliationId = await createSupersAgentStableIdentityHash(
        receiptCore,
      );
...
async function resolveSupersAgentWorktreeRoot(
  repositoryDir: string,
): Promise<string> {
  const temporaryRoot = await Deno.realPath(Deno.env.get("TMPDIR") ?? "/tmp");
  const repositoryPathHash = (await rawSha256(encoder.encode(repositoryDir)))
    .slice(0, 12);
  const managedRoot = `${temporaryRoot}/supers-agent-worktrees/${
    baseName(repositoryDir)
  }-${repositoryPathHash}`;
  ...
  while (pending.length > 0) {
    ...
    for (const entry of entries) {
      ...
      if (!Number.isSafeInteger(logicalBytes)) {
        throw new Error(
          "agent worktree logical size exceeds the safe integer range",
        );
      }
    }
  }
...
type CliAgentInjectedGlobalArgs = z.infer<
  typeof CliAgentInjectedGlobalArgsSchema
>;
  ...
  methods: [
    ...
    },
    {
      verifySupersAgentWorktreeUnchanged: {
        ...
        arguments: VerifySupersAgentWorktreeUnchangedArgsSchema.extend(
          CliAgentInjectedGlobalArgsSchema.shape,
        ),
        execute: async (
          args:
            & VerifySupersAgentWorktreeUnchangedArgs
            & z.infer<typeof CliAgentInjectedGlobalArgsSchema>,
          context: SupersAgentWorktreeMethodContext,
        ) => {
          const result = await operations.verifySupersAgentWorktreeUnchanged(
            VerifySupersAgentWorktreeUnchangedArgsSchema.parse(
              stripCliAgentInjectedGlobals(args),
            ),
            context,
          );
    ...
    },
    {
      verifySupersAgentWorktreeCommit: {
        ...
        arguments: VerifySupersAgentWorktreeCommitArgsSchema.extend(
          CliAgentInjectedGlobalArgsSchema.shape,
        ),
        execute: async (
          args:
          ...
          const result = await operations.verifySupersAgentWorktreeCommit(
            VerifySupersAgentWorktreeCommitArgsSchema.parse(
              stripCliAgentInjectedGlobals(args),
            ),
            context,
          );
    ...
    },
    {
      verifySupersAgentIntegration: {
        ...
        arguments: VerifySupersAgentIntegrationArgsSchema.extend(
          CliAgentInjectedGlobalArgsSchema.shape,
        ),
        execute: async (
          args:
            & VerifySupersAgentIntegrationArgs
            & z.infer<typeof CliAgentInjectedGlobalArgsSchema>,
          context: SupersAgentWorktreeMethodContext,
        ) => {
          const result = await operations.verifySupersAgentIntegration(
            VerifySupersAgentIntegrationArgsSchema.parse(
              stripCliAgentInjectedGlobals(args),
            ),
            context,
          );
    ...
    },
    {
      removeSupersAgentWorktree: {
        ...
        arguments: RemoveSupersAgentWorktreeArgsSchema.extend(
          CliAgentInjectedGlobalArgsSchema.shape,
        ),
        execute: async (
          args:
            & RemoveSupersAgentWorktreeArgs
            & z.infer<typeof CliAgentInjectedGlobalArgsSchema>,
          context: SupersAgentWorktreeMethodContext,
        ) => {
          const result = await operations.removeSupersAgentWorktree(
            RemoveSupersAgentWorktreeArgsSchema.parse(
              stripCliAgentInjectedGlobals(args),
            ),
            context,
          );
    ...
    },
    {
      reconcileSupersAgentWorktrees: {
        ...
        execute: async (
          args:
            & ReconcileSupersAgentWorktreesArgs
            & z.infer<typeof CliAgentInjectedGlobalArgsSchema>,
          context: SupersAgentWorktreeMethodContext,
        ) => {
