...
import type { DexPlanApplierMethodContext, DexPlanApplyArgs } from './dex-plan-applier-adapter.ts';
...
export const model = {
	...
	resources: {
		checkpoint: {
			description: 'Durable roll-forward state updated after every confirmed Dex mutation',
			schema: DexPlanApplyCheckpointSchema,
			lifetime: 'infinite',
			garbageCollection: 100
		},
		receipt: {
			description: 'Versioned terminal outcome for each Dex plan application attempt',
			schema: DexPlanApplyReceiptSchema,
			lifetime: 'infinite',
			garbageCollection: 100
		},
		result: {
			description: 'Stable client-reference to Dex-id mapping for an applied plan',
	...
	methods: {
		'apply-plan': {
			description: 'Apply one fully approved Dex plan serially with checkpointed recovery',
