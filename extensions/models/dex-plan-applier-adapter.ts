...
export const DEX_PLAN_APPLIER_VERSION = '2026.08.05.1';
...
const DexCreatedTaskDescriptionSchema = z.string().min(1).max(MAX_DEX_CONTENT_LENGTH);
...
export const DexApprovedPlanSchema = z
	...
	.refine(
		(plan) => plan.tasks.length + (plan.epic === undefined ? 0 : 1) <= MAX_PLAN_NODES,
...
export type DexPlanApplierGlobalArgs = z.infer<typeof DexPlanApplierGlobalArgsSchema>;
...
const RetryDispositionSchema = z.enum(['retry', 'do-not-retry', 'manual-review']);
...
async function createPlanIdentity(
	...
	const planHash = await sha256Hex(
		JSON.stringify(canonicalize({ ownerToken, plan: normalizedPlanForHash(plan) }))
	);
	const idempotencyKey = await sha256Hex(`dex-plan-applier\0v1\0${plan.planId}`);
	return {
		planHash,
		idempotencyKey,
...
function requireReferences(nodes: PlanNode[], nodesByRef: Map<string, PlanNode>): void {
	for (const node of nodes) {
		const parentRef = node.parent.kind === 'reference' ? node.parent.clientRef : null;
...
function validateDexGraphIntegrity(
	...
	for (const task of tasks) {
		if (tasksById.has(task.id)) {
			throw graphIntegrityError(`Dex task id ${task.id} is duplicated`, retryDisposition);
		}
		tasksById.set(task.id, task);
	}
	for (const task of tasks) {
		requireUniqueRelations(task.id, 'children', task.children, retryDisposition);
		requireUniqueRelations(task.id, 'blockedBy', task.blockedBy, retryDisposition);
...
function requireApprovedExistingContent(task: RawDexTask, node: ExistingPlanNode): void {
	if (
...
function requireApprovedCreatedContent(task: RawDexTask, node: CreatePlanNode): void {
	if (
		task.name !== node.name ||
...
function validateMappedCreatedTasks(
	...
	const tasksById = new Map(currentTasks.map((task) => [task.id, task]));
	for (const node of nodes) {
		if (node.kind !== 'create' || priorMapping[node.clientRef] === undefined) continue;
		const task = tasksById.get(priorMapping[node.clientRef]);
		if (task === undefined) continue;
		const approvedParentId =
			node.parent.kind === 'root' ? null : resolvedIdsByRef.get(node.parent.clientRef);
...
function graphIdForRef(clientRef: string, resolvedIdsByRef: Map<string, string>): string {
...
function validateHierarchy(
	...
	const parentById = new Map(currentTasks.map((task) => [task.id, task.parent_id]));
	...
	for (const node of nodes) {
		...
		parentById.set(
			id,
			node.parent.kind === 'root' ? null : graphIdForRef(node.parent.clientRef, resolvedIdsByRef)
		);
	}
	...
	for (const startId of affected) {
		...
		while (id !== null) {
			if (seen.has(id)) {
				throw planError('hierarchy-cycle', `Hierarchy cycle reaches ${startId}`);
			}
			seen.add(id);
...
function validateBlockers(
	...
	const blockersById = new Map(currentTasks.map((task) => [task.id, [...task.blockedBy]]));
	const affected: string[] = [];
	for (const node of nodes) {
		const id = graphIdForRef(node.clientRef, resolvedIdsByRef);
		affected.push(id);
		const additions = node.blockedBy.map((ref) => graphIdForRef(ref, resolvedIdsByRef));
		blockersById.set(
			id,
			node.kind === 'create'
				? additions
...
function createOrder(nodes: PlanNode[], nodesByRef: Map<string, PlanNode>): CreatePlanNode[] {
	const creates = nodes.filter((node): node is CreatePlanNode => node.kind === 'create');
	...
	for (const node of creates) {
		...
		children.set(parent.clientRef, [...(children.get(parent.clientRef) ?? []), node.clientRef]);
	}
	const ready = creates
	...
	if (ordered.length !== creates.length) {
		throw planError('hierarchy-cycle', 'Planned create hierarchy contains a cycle');
	}
	return ordered;
}
...
function preflightPlan(
	...
	const resolvedIdsByRef = resolveExistingTasks(nodes, currentTasks, priorMapping);
	validateMappedCreatedTasks(nodes, currentTasks, priorMapping, resolvedIdsByRef);
...
async function initialOperations(
	...
	for (const node of [...nodes].sort((left, right) =>
		...
		if (node.kind === 'attachExisting') {
			operations.push({
				operationKey: await operationKey(idempotencyKey, node.clientRef, 'detach'),
		...
		operations.push({
			operationKey: await operationKey(idempotencyKey, node.clientRef, 'hierarchy'),
			clientRef: node.clientRef,
			phase: 'hierarchy',
			status:
				node.kind === 'attachExisting' && node.parent.kind === 'preserve' ? 'preserved' : 'pending',
			dexTaskId: null
		});
		operations.push({
			operationKey: await operationKey(idempotencyKey, node.clientRef, 'blockers'),
			clientRef: node.clientRef,
			phase: 'blockers',
...
function checkpointOperation(
	...
	const operation = checkpoint.operations.find(
		(candidate) => candidate.clientRef === clientRef && candidate.phase === phase
	);
...
function desiredParentId(
	...
	if (node.parent.kind !== 'reference') {
		throw planError('unexpected-failure', 'Unsupported parent directive', 'retry');
	}
...
async function applyExistingDetachment(
	...
	for (const node of preflight.nodes
		.filter((candidate): candidate is ExistingPlanNode => candidate.kind === 'attachExisting')
		.sort((left, right) => left.clientRef.localeCompare(right.clientRef))) {
		...
		const currentTask = (await dependencies.commandAdapter.listTasks(context.repoDir)).find(
		...
		if (node.parent.kind === 'preserve') {
			checkpointOperation(checkpoint, node.clientRef, 'detach', 'preserved', taskId);
			continue;
		}
		const finalParentId =
			node.parent.kind === 'root' ? null : checkpoint.taskIdsByClientRef[node.parent.clientRef];
		if (
		...
		try {
			...
			const refreshed = (await dependencies.commandAdapter.listTasks(context.repoDir)).find(
				(candidate) => candidate.id === taskId
			);
		...
		checkpointOperation(checkpoint, node.clientRef, 'detach', 'succeeded', taskId);
...
async function applyCreateHierarchy(
	...
	for (const node of preflight.createOrder) {
		...
		const parentId = desiredParentId(node, checkpoint.taskIdsByClientRef, undefined);
		if (currentMapping !== undefined) {
			const existing = (await dependencies.commandAdapter.listTasks(context.repoDir)).find(
			...
			checkpointOperation(checkpoint, node.clientRef, 'hierarchy', 'succeeded', existing.id);
			continue;
		}
		...
		checkpoint.taskIdsByClientRef = sortedMapping(checkpoint.taskIdsByClientRef);
		checkpointOperation(checkpoint, node.clientRef, 'hierarchy', 'succeeded', task.id);
		checkpoint.updatedAt = dependencies.now();
		await writeCheckpoint(context, checkpointName, checkpoint);
	}
}
...
async function applyExistingHierarchy(
	...
	for (const node of preflight.nodes
		...
		if (node.parent.kind === 'preserve') {
			checkpointOperation(checkpoint, node.clientRef, 'hierarchy', 'preserved', taskId);
			continue;
		}
		...
		checkpointOperation(checkpoint, node.clientRef, 'hierarchy', 'succeeded', taskId);
		checkpoint.updatedAt = dependencies.now();
		await writeCheckpoint(context, checkpointName, checkpoint);
	}
}
...
async function applyBlockers(
	...
	for (const node of [...preflight.nodes].sort((left, right) =>
		left.clientRef.localeCompare(right.clientRef)
	)) {
		const taskId = checkpoint.taskIdsByClientRef[node.clientRef];
		if (taskId === undefined) {
			throw planError('unexpected-failure', `Missing task mapping ${node.clientRef}`, 'retry');
		}
		const currentTask = (await dependencies.commandAdapter.listTasks(context.repoDir)).find(
			(candidate) => candidate.id === taskId
		);
		...
		const desiredBlockers = node.blockedBy.map((ref) => {
			...
			if (blockerId === undefined) {
				throw planError('missing-client-reference', `Blocker ${ref} has no Dex mapping`, 'retry');
			}
			return blockerId;
		});
		const missing = desiredBlockers.filter((blockerId) => !task.blockedBy.includes(blockerId));
		if (missing.length > 0) {
			try {
				task = await dependencies.commandAdapter.updateTask(context.repoDir, {
					id: taskId,
				...
				if (
					refreshed === undefined ||
					!desiredBlockers.every((blockerId) => refreshed.blockedBy.includes(blockerId))
				) {
					throw planError(
						'dex-update-failed',
		...
		if (!desiredBlockers.every((blockerId) => task.blockedBy.includes(blockerId))) {
			throw planError(
				'verification-failed',
...
function requireExpectedExisting(task: RawDexTask, node: ExistingPlanNode): void {
	if (
		task.name !== node.expected.name ||
		task.description !== node.expected.description ||
		task.priority !== node.expected.priority
...
async function verifyFinalGraph(
	...
	try {
		...
		throw planError('dex-list-failed', 'Could not verify final Dex graph', 'retry', null, error);
	}
	...
	for (const node of preflight.nodes) {
		...
		const desiredBlockers = node.blockedBy.map((ref) => checkpoint.taskIdsByClientRef[ref]);
		if (desiredBlockers.some((blockerId) => blockerId === undefined)) {
			throw planError('verification-failed', 'A blocker mapping is missing', 'manual-review');
		}
		const blockerIds = desiredBlockers.filter((id): id is string => id !== undefined);
		...
		if (
			node.kind === 'create' &&
			[...task.blockedBy].sort().join('\0') !== [...blockerIds].sort().join('\0')
		) {
			throw planError(
				'verification-failed',
...
async function writeReceipt(
	...
	try {
		return await context.writeResource('receipt', name, DexPlanApplyReceiptSchema.parse(receipt));
	} catch (error) {
		throw planError(
			'resource-write-failed',
...
function parseMcpPayload(value: unknown, operation: string): unknown {
	...
	try {
		...
		throw planError('mcp-protocol-invalid', `Invalid ${operation} response`, 'retry', null, error);
	}
	...
	if (text === undefined) {
		throw planError('mcp-protocol-invalid', `${operation} returned no JSON payload`, 'retry');
	}
	let payload: unknown;
	try {
...
async function callDexMcpTool(
	...
	if (output.code !== 0) {
		throw planError(
			'mcp-command-failed',
			`dex mcp exited ${output.code}${stderr ? `: ${stderr.slice(0, OUTPUT_EXCERPT_LENGTH)}` : ''}`,
			'retry'
		);
	}
...
function parseRawTask(value: unknown): RawDexTask {
	try {
		...
		throw planError('dex-response-invalid', 'Dex returned an invalid task', 'retry', null, error);
	}
}
...
function createDenoDexPlanCommandAdapter(): DexPlanCommandAdapter {
	return {
		...
		createTask: async (cwd, args) => parseRawTask(await callDexMcpTool(cwd, 'create_task', args)),
		updateTask: async (cwd, args) => parseRawTask(await callDexMcpTool(cwd, 'update_task', args))
	};
}
...
function mappingsEqual(left: Record<string, string>, right: Record<string, string>): boolean {
	return JSON.stringify(sortedMapping(left)) === JSON.stringify(sortedMapping(right));
}
...
function resultMappingsEqual(
	...
	return JSON.stringify(actual) === JSON.stringify(expectedResultMappings(preflight, checkpoint));
}
...
async function persistRepositoryLockPlanFailure(
	...
	const attempt = Math.max(checkpoint?.attempt ?? 0, priorReceipt?.attempt ?? 0) + 1;
...
async function executeDexPlanApplyLocked(
	...
	try {
		...
		prior = priorRaw === null ? null : DexPlanApplyCheckpointSchema.parse(priorRaw);
		const priorReceipt = samePlanReceipt(
			await context.readResource(identity.receiptName),
			plan,
			identity,
		...
		invocationAttempt = Math.max(prior?.attempt ?? 0, priorReceipt?.attempt ?? 0) + 1;
		...
		try {
			currentTasks = await dependencies.commandAdapter.listTasks(context.repoDir);
		} catch (error) {
			throw planError('dex-list-failed', 'Could not inventory Dex tasks', 'retry', null, error);
		}
		...
		if (prior?.status === 'succeeded') {
			...
			if (resultRaw === null) {
				throw planError('resource-write-failed', 'Succeeded plan result is missing', 'retry');
			}
			const result = requireResultForCheckpoint(resultRaw, replayCheckpoint, preflight);
			...
			const receipt = isCurrentSuccessReceipt(receiptRaw, replayCheckpoint, identity)
				? DexPlanApplyReceiptSchema.parse(receiptRaw)
				: successReceipt(replayCheckpoint, identity, dependencies.now());
			const resultHandle = await writeResult(context, identity.resultName, result);
			const receiptHandle = await writeReceipt(context, identity.receiptName, receipt);
		...
		checkpoint =
			prior === null
				? DexPlanApplyCheckpointSchema.parse({
						...
						operations: await initialOperations(preflight.nodes, identity.idempotencyKey)
					})
				: DexPlanApplyCheckpointSchema.parse({
						...prior,
						attempt: invocationAttempt,
		...
		await applyBlockers(preflight, checkpoint, identity.checkpointName, context, dependencies);
		await verifyFinalGraph(preflight, checkpoint, context, dependencies.commandAdapter);
		...
		checkpoint.updatedAt = dependencies.now();
		const checkpointHandle = await writeCheckpoint(context, identity.checkpointName, checkpoint);
		...
		const failureMapping =
			checkpoint?.taskIdsByClientRef ??
			(!isConflict && prior?.planHash === identity.planHash ? prior.taskIdsByClientRef : {});
		try {
			await persistFailureReceipt(
				context,
				identity,
...
export function executeDexPlanApply(
	...
	return withRepositoryInvocationLock(context.repoDir, async () => {
		const identity = await createPlanIdentity(plan, context.globalArgs.ownerToken);
		...
		try {
			return await dependencies.repositoryLock.runExclusive(context.repoDir, async () => {
				const result = await executeDexPlanApplyLocked(plan, identity, context, dependencies);
				committedResult = result;
				return result;
			});
		} catch (error) {
			if (committedResult !== null) {
				context.logger.warning('Dex repository lock cleanup failed after plan commit', {
					planId: plan.planId
				});
			...
			try {
				await persistRepositoryLockPlanFailure(plan, identity, context, dependencies, failure);
			} catch (writeError) {
