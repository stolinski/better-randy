...
class FakeWorktreeRepository implements SupersAgentWorktreeDependencies {
  ...
  realPath(path: string): Promise<string> {
    ...
    return Promise.resolve(
      this.untrackedInfo.get(relative)?.canonicalPath ?? path,
    );
  }
  ...
  pathExists(path: string): Promise<boolean> {
    ...
    return Promise.resolve(
      this.conflictPathExists && path.includes("-supers-agent-"),
    );
  }
  ...
  fileInfo(
    path: string,
  ): Promise<{ isFile: boolean; isSymlink: boolean; size: number }> {
    const prefix = `${this.worktreePath}/`;
    const relative = path.startsWith(prefix) ? path.slice(prefix.length) : path;
  ...
  private runCentralGit(
    ...
    if (command === "rev-parse --path-format=absolute --git-dir") {
      return gitResult(
        true,
        this.centralIsLinkedWorktree
          ? "/primary/.git/worktrees/repo\n"
          : "/repo/.git\n",
      );
    }
    if (command === "rev-parse --path-format=absolute --git-common-dir") {
      return gitResult(
        true,
        this.centralIsLinkedWorktree ? "/primary/.git\n" : "/repo/.git\n",
      );
    }
    ...
    if (
      command ===
        `rev-list --min-parents=2 ${BASE_REVISION}..${this.worktreeHead}`
    ) {
    ...
    if (command === `rev-list --parents -n 1 ${INTEGRATED_REVISION}`) {
      return gitResult(
        true,
        `${INTEGRATED_REVISION} ${this.integrationParent}\n`,
      );
    }
    ...
    if (
      ...
      return gitResult(
        true,
        this.integrationNameStatus ??
          `${this.changedPathStatus}\0${this.changedPath}\0`,
      );
    }
    if (
      command ===
      ...
      return gitResult(
        ...
        this.integrationDiffBytes ??
          new TextEncoder().encode(
            `diff --git a/${this.changedPath} b/${this.changedPath}\n`,
          ),
      );
    }
    ...
    return gitResult(
      false,
      "",
      `unexpected central git command: ${command}`,
      128,
    );
  }
  ...
  private runWorktreeGit(command: string): SupersAgentGitResult {
    ...
    if (command === "ls-files --others --exclude-standard -z") {
      return gitResult(
        true,
        [...this.untracked.keys()].join("\0") +
          (this.untracked.size > 0 ? "\0" : ""),
      );
    }
    if (command === "status --porcelain=v1 --untracked-files=all -z") {
    ...
    if (
      command === `merge-base --is-ancestor ${BASE_REVISION} ${COMMIT_REVISION}`
    ) {
      return gitResult(this.baseIsAncestor);
    }
    if (
      ...
      return gitResult(
        true,
        Array.from(
          { length: this.changedPathCount },
          (_, index) =>
            `${this.changedPathStatus}\0${
              index === 0 ? this.changedPath : `src/generated-${index}.ts`
            }\0`,
        ).join(""),
      );
    }
    if (
      ...
      return gitResult(
        true,
        `${this.changedPathMode} ${
          this.changedPathMode === "160000" ? "commit" : "blob"
        } ${OTHER_REVISION}\t${this.changedPath}\0`,
      );
    }
    ...
    if (command === `ls-tree --full-tree -r -z ${COMMIT_REVISION}`) {
      ...
      if (this.mutateTreeOnSecondCollection && this.treeCollectionCount >= 2) {
        return gitResult(
          true,
          `100644 blob ${COMMIT_TREE}\tchanged-after-check.ts\0`,
        );
      }
      return gitResult(
        true,
    ...
    if (
      command ===
        `diff --binary --full-index ${BASE_REVISION} ${COMMIT_REVISION} --`
    ) {
    ...
    return gitResult(
      false,
      "",
      `unexpected worktree git command: ${command}`,
      128,
    );
  }
}
    ...
    context: {
      ...
      dataRepository: {
        getContent: (type, modelId, name) => {
          const value = externalResources.get(
...
async function integrationFixture() {
  ...
  const commitReceipt = (
    await operations.verifySupersAgentWorktreeCommit(
      commitVerificationArgs(claim),
      context,
    )
  ).resource;
  ...
  const args = {
    commitReceiptName:
      `supers-agent-worktree-commit-${commitReceipt.receiptId}`,
...
Deno.test("worktree authority resources retain bounded infinite-lifetime histories", () => {
  for (const [name, resource] of Object.entries(extension.resources)) {
    ...
    assert.equal(
      resource.garbageCollection,
      name === "supers-agent-worktree-reconciliation" ? 32 : 1,
    );
  }
});
...
Deno.test("clean exact HEAD prepares, verifies, and removes an isolated worktree", async () => {
  ...
  assert.equal(
    claim.worktreePath.startsWith(
      "/tmp/supers-agent-worktrees/repo-supers-agent-",
    ),
    true,
  );
  assert.equal(
    claim.attachedBranch.startsWith("supers-agent/sentry-reproduction/"),
    true,
  );
  ...
  const removed = (await operations.removeSupersAgentWorktree(
    { claimId: claim.claimId },
    context,
  ))
    .resource;
...
Deno.test("legacy sibling worktree claims remain replayable after managed-root migration", async () => {
  ...
  const legacyClaim =
    (await operations.prepareSupersAgentWorktree(PREPARE_ARGS, context))
      .resource;
  assert.equal(
    legacyClaim.worktreePath.startsWith("/repo-supers-agent-"),
    true,
  );
...
Deno.test("reconciliation removes a final unchanged failed worktree and records its size", async () => {
  ...
  const receipt = (
    await operations.reconcileSupersAgentWorktrees({
...
Deno.test("reconciliation removes patch-equivalent commits and stale registrations", async () => {
  for (const staleRegistration of [false, true]) {
    ...
    const claim =
      (await operations.prepareSupersAgentWorktree(PREPARE_ARGS, context))
        .resource;
    repository.worktreeHead = staleRegistration
      ? BASE_REVISION
      : COMMIT_REVISION;
    repository.supersededCherry = staleRegistration
      ? ""
      : `- ${COMMIT_REVISION}\n`;
...
Deno.test("committed worktree verification persists objective evidence after the central branch advances", async () => {
  ...
  const first =
    (await operations.verifySupersAgentWorktreeCommit(args, context)).resource;
...
Deno.test("committed worktree verification hashes raw bytes and detects verification races", async () => {
  ...
  assert.notEqual(
    digests[0],
    digests[1],
    "distinct invalid UTF-8 byte sequences must retain distinct digests",
  );
...
Deno.test("committed worktree verification performs a final clean identity check before persistence", async () => {
  ...
  assert.equal(
    writes.some((name) => name.startsWith("supers-agent-worktree-commit-")),
    false,
  );
});
  ...
  > = [
    {
      configure: (repository) => {
        repository.changedPathCount = 257;
      },
      expected: /256-path safety limit/,
    },
    {
      configure: (repository) => {
        repository.changedBlobSize = 8 * 1024 * 1024 + 1;
      },
      expected: /changed path exceeds/,
    },
    {
      configure: (repository) => {
        repository.treeListingBytes = new Uint8Array(8 * 1024 * 1024 + 1);
      },
      expected: /ls-tree output exceeds/,
    },
    {
      configure: (repository) => {
        repository.binaryDiffBytes = new Uint8Array(32 * 1024 * 1024 + 1);
      },
      expected: /diff output exceeds/,
    },
  ];
  for (const testCase of cases) {
    const { repository, context, resources } = fixture();
...
Deno.test("committed worktree verification binds claim, invocation, prompt, model, and central base", async () => {
  const cases: Array<{
    ...
    overrides?: (
      claim: SupersAgentWorktreeClaim,
    ) => Partial<VerifySupersAgentWorktreeCommitArgs>;
    expected: RegExp;
  }> = [
    {
...
Deno.test("serialized integration verification persists a strict Factory receipt and replays exactly", async () => {
  ...
  const first =
    (await operations.verifySupersAgentIntegration(args, context)).resource;
  ...
  const integrationName =
    `supers-agent-integration-supers-101-${INTEGRATED_REVISION}`;
...
Deno.test("serialized integration verification ignores unrelated dirt but rejects revision drift", async () => {
  {
    const { repository, context, operations, args } =
      await integrationFixture();
...
Deno.test("serialized integration verification accepts an unrelated target parent but rejects changed paths and patch drift", async () => {
  ...
  for (const scenario of ["paths", "diff"] as const) {
    const { repository, context, operations, args } =
      await integrationFixture();
    if (scenario === "paths") {
      repository.integrationNameStatus = `A\0other.test.ts\0`;
    }
    if (scenario === "diff") {
      repository.integrationDiffBytes = new TextEncoder().encode("wrong");
    }
    await assert.rejects(() =>
      operations.verifySupersAgentIntegration(args, context)
    );
...
Deno.test("serialized integration verification rejects forged commit and cherry-pick evidence", async () => {
  {
    const { repository, context, resources, operations, args } =
      await integrationFixture();
    const forged = {
      ...resources.get(args.commitReceiptName)!,
  ...
  for (
    const cherryPick of [
      {
        commits: [COMMIT_REVISION, OTHER_REVISION],
        conflict: false,
        raw: "extra",
      },
...
Deno.test("unrelated central dirt does not block worktree preparation or commit verification", async () => {
  ...
  repository.worktreeHead = COMMIT_REVISION;
  recordSuccessfulInvocation(resources, claim);
  const receipt = (await operations.verifySupersAgentWorktreeCommit(
    commitVerificationArgs(claim),
    context,
  )).resource;
  assert.equal(receipt.commitRevision, COMMIT_REVISION);
...
Deno.test("worktree preparation tolerates central advancement but rejects a divergent base", async () => {
  {
    const { repository, context } = fixture();
    repository.centralHeads = [OTHER_REVISION];
    const operations = createSupersAgentWorktreeOperations(repository);
  ...
  }
  {
    ...
    await assert.rejects(
      () => operations.prepareSupersAgentWorktree(PREPARE_ARGS, context),
      /does not contain worktree base revision/,
    );
    assert.equal(repository.addCalls, 0);
  }
});
...
Deno.test("repository hashing remains byte-compatible for accepted regular files", async () => {
  ...
  assert.equal(
    await createSupersAgentRepositoryStateHash(
...
Deno.test("repository hashing rejects unsafe untracked filesystem entries", async (test) => {
  ...
  await test.step("symlink", async () => {
    await rejectUntracked((repository) => {
      ...
      repository.untrackedInfo.set("link", {
        isFile: false,
        isSymlink: true,
        size: 4,
      });
  ...
  await test.step("non-regular file", async () => {
    await rejectUntracked((repository) => {
      ...
      repository.untrackedInfo.set("pipe", {
        isFile: false,
        isSymlink: false,
        size: 0,
      });
  ...
  await assert.rejects(
    () =>
      operations.removeSupersAgentWorktree(
        {
          claimId: claim.claimId,
          authorization: { ...authorization, fingerprint: "f".repeat(64) },
        },
        context,
      ),
  ...
  await assert.rejects(
    () =>
      operations.removeSupersAgentWorktree(
        {
          claimId: claim.claimId,
          authorization: { ...authorization, receiptName: "wrong-receipt" },
        },
        context,
      ),
...
Deno.test("cleanup replay does not issue a second removal or resource write", async () => {
  ...
  const first = (await operations.removeSupersAgentWorktree(
    { claimId: claim.claimId },
    context,
  ))
    .resource;
  const writesAfterRemoval = writes.length;
  const replay = (await operations.removeSupersAgentWorktree(
    { claimId: claim.claimId },
    context,
  ))
    .resource;
...
Deno.test("cleanup rejects an already-absent worktree on its first attempt", async () => {
  ...
  assert.equal(
    [...resources.keys()].some((name) =>
      name.startsWith("supers-agent-worktree-removal-intent-")
    ),
    false,
  );
});
...
Deno.test(
  ...
  async () => {
    ...
    try {
      ...
      const baseRevision = await requireRealGit(repositoryDir, [
        "rev-parse",
        "HEAD",
      ]);
      const resources = new Map<string, Record<string, unknown>>();
      ...
      const realDependencies: SupersAgentWorktreeDependencies = {
        ...
        measureDirectory: () =>
          Promise.resolve({ logicalBytes: 0, complete: true }),
      ...
      assert.equal(
        claim.worktreePath.startsWith(
          `${managedWorktreeRoot}/primary-supers-agent-`,
        ),
        true,
      );
      const registered = await requireRealGit(context.repoDir, [
        "worktree",
        "list",
        "--porcelain",
      ]);
      ...
      const foreignContext: SupersAgentWorktreeMethodContext = {
        ...context,
        readResource: (name) =>
          Promise.resolve(foreignResources.get(name) ?? null),
        writeResource: (_spec, name, data) => {
      ...
      await assert.rejects(
        () => Deno.realPath(claim.worktreePath),
      ...
      assert.equal(
        afterRemoval.includes(`branch refs/heads/${claim.attachedBranch}`),
        false,
      );
      assert.equal(
      ...
      await Deno.mkdir(`${committedClaim.worktreePath}/extensions/models`, {
        recursive: true,
      });
      ...
      assert.equal(
        await Deno.readTextFile(`${repositoryDir}/tracked.txt`),
        "central dirty\n",
      );
      ...
      await operations.removeSupersAgentWorktree(
        {
          ...
          authorization: {
            kind: "committed",
            receiptName:
              `supers-agent-worktree-commit-${committedReceipt.receiptId}`,
      ...
      await assert.rejects(
        () => Deno.realPath(committedClaim.worktreePath),
        Deno.errors.NotFound,
      );
