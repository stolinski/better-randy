...
async function changeResourceName(prefix: string, workItem: string): Promise<string> {
	return `${prefix}-${(await sha256Hex(workItem)).slice(0, 32)}`;
}

async function runGit(context: MethodContext, args: string[]): Promise<Uint8Array> {
	...
	if (result.code !== 0) {
		...
		throw new Error(`git ${args[0]} exited ${result.code}: ${stderr.slice(0, 800)}`);
	}
...
async function readGitHead(context: MethodContext): Promise<string> {
	return GitHeadSchema.parse(
		textDecoder.decode(await runGit(context, ['rev-parse', '--verify', 'HEAD'])).trim()
	);
}
...
async function readCurrentTreeState(context: MethodContext): Promise<CurrentTreeState> {
	...
	const [unstagedDiff, stagedDiff, status, untrackedOutput] = await Promise.all([
		...
		runGit(context, ['diff', '--cached', '--binary', '--no-ext-diff', '--no-textconv']),
		runGit(context, ['status', '--porcelain=v1', '-z', '--untracked-files=all']),
...
async function readChangeBaseline(
	...
	if (!resource) throw new Error(`No change baseline recorded for work item ${workItem}`);
...
export const model = {
	...
	resources: {
		...
		'change-baseline': {
			description: 'Per-work-item trusted Git HEAD captured before Factory implementation begins',
	...
	methods: {
		...
		'record-deep-audit': {
			...
			arguments: z.object({
				reportPath: z.string().describe('Path to the audit report JSON, relative to the repo root')
			}),
			execute: async (args: { reportPath: string }, context: MethodContext) => {
				...
				try {
					...
					throw new Error(`Could not read audit report at ${absolutePath}`, { cause: error });
				}
		...
		'capture-change-baseline': {
			description: 'Capture the current Git HEAD as the trusted baseline for a work item',
			arguments: z.object({ workItem: z.string().min(1) }),
			execute: async (args: { workItem: string }, context: MethodContext) => {
				const resourceName = await changeResourceName('change-baseline', args.workItem);
				...
				const handle = await context.writeResource('change-baseline', resourceName, baseline);
				return { dataHandles: [handle] };
			}
		},
		'classify-change': {
			...
			execute: async (args: { workItem: string }, context: MethodContext) => {
				...
				const resourceName = await changeResourceName('change-impact', args.workItem);
				const handle = await context.writeResource('change-impact', resourceName, canonicalReport);
				return { dataHandles: [handle] };
			}
		},
		'assert-change-state': {
			description: 'Fail when the current content-sensitive Git tree fingerprint has drifted',
			arguments: z.object({ expectedFingerprint: TreeFingerprintSchema }),
			execute: async (args: { expectedFingerprint: string }, context: MethodContext) => {
				const currentTree = await readCurrentTreeState(context);
