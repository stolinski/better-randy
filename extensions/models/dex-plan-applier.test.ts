...
function oneTaskPlan(overrides: Partial<DexApprovedPlan> = {}): DexApprovedPlan {
...
class FakeDexPlanCommandAdapter implements DexPlanCommandAdapter {
	...
	async createTask(_cwd: string, args: DexMcpCreateTaskArguments): Promise<RawDexTask> {
	...
	async updateTask(_cwd: string, args: DexMcpUpdateTaskArguments): Promise<RawDexTask> {
	...
	#syncParent(taskId: string, previousParentId: string | null, nextParentId: string | null): void {
...
function fixtureContext(
	...
	return {
		...
		context: {
			...
			readResource: (name) => Promise.resolve(sharedResources.get(name) ?? null),
...
function dependencies(commandAdapter: DexPlanCommandAdapter): DexPlanApplierDependencies {
	return {
		commandAdapter,
...
Deno.test('model exposes one method and the three durable resource types', () => {
	...
	assert.equal(model.version, '2026.08.05.1');
	assert.deepEqual(Object.keys(model.methods), ['apply-plan']);
	assert.deepEqual(Object.keys(model.resources).sort(), ['checkpoint', 'receipt', 'result']);
});
...
Deno.test('one task preserves exact priority and owner attribution', async () => {
	...
	await executeDexPlanApply({ plan: oneTaskPlan() }, fixture.context, dependencies(adapter));
	...
	const result = DexPlanApplyResultSchema.parse(latestWrite(fixture.writes, 'result').data);
	assert.equal(result.ownerToken, ownerToken);
	assert.equal(adapter.tasks.get(result.taskIdsByClientRef['implementation-task'])?.priority, 73);
	const receipt = DexPlanApplyReceiptSchema.parse(latestWrite(fixture.writes, 'receipt').data);
...
Deno.test('post-commit repository lock cleanup failure preserves plan success', async () => {
	...
	assert.equal(fixture.writes.filter((write) => write.specName === 'receipt').length, 1);
	assert.equal(
		DexPlanApplyReceiptSchema.parse(latestWrite(fixture.writes, 'receipt').data).status,
		'succeeded'
	);
	assert.equal(
		DexPlanApplyCheckpointSchema.parse(latestWrite(fixture.writes, 'checkpoint').data).status,
		'succeeded'
	);
	assert.deepEqual(
...
Deno.test('blockers are applied after every task has a stable mapping', async () => {
	...
	assert.deepEqual(adapter.updateCalls, [{ id: 'created-002', add_blocked_by: ['created-001'] }]);
	assert.deepEqual(adapter.tasks.get('created-002')?.blockedBy, ['created-001']);
});
...
Deno.test('existing task attachment reparents without changing content or priority', async () => {
	...
	assert.equal(adapter.tasks.get(existing.id)?.description, existing.description);
...
Deno.test('retry resumes after a confirmed partial create without duplicates', async () => {
	...
	const failed = DexPlanApplyCheckpointSchema.parse(latestWrite(fixture.writes, 'checkpoint').data);
	...
	assert.equal(adapter.createCalls.filter((call) => call.name === 'Alpha').length, 1);
	assert.equal(adapter.createCalls.filter((call) => call.name === 'Beta').length, 2);
});
...
Deno.test('retry rejects an externally added blocker before further mutation', async () => {
	...
	assert.equal(
		DexPlanApplyReceiptSchema.parse(latestWrite(fixture.writes, 'receipt').data).errorCode,
		'existing-task-drift'
	);
});
...
Deno.test('ambiguous uncertain create fails closed for manual review', async () => {
	...
	const fixture = fixtureContext();
	const failure = await assertPlanFailure(
		executeDexPlanApply({ plan: oneTaskPlan() }, fixture.context, dependencies(adapter)),
		'recovery-ambiguous'
	);
...
Deno.test(
	...
	async () => {
		...
		const replay = await executeDexPlanApply({ plan }, fixture.context, dependencies(adapter));
		assert.equal(adapter.createCalls.length, 1);
		assert.equal(adapter.tasks.size, 1);
		assert.deepEqual(
		...
		assert.equal(
			DexPlanApplyCheckpointSchema.parse(latestWrite(fixture.writes, 'checkpoint').data).attempt,
			2
		);
	}
);
...
Deno.test('replay attempts remain monotonic across failed verification and repair', async () => {
	...
	assert.equal(
		DexPlanApplyReceiptSchema.parse(latestWrite(fixture.writes, 'receipt').data).attempt,
		2
	);
...
Deno.test(
	...
	async () => {
		...
		const checkpointBefore = DexPlanApplyCheckpointSchema.parse(checkpointBeforeReplay);
		...
		await executeDexPlanApply({ plan }, fixture.context, dependencies(adapter));
		assert.deepEqual(
			fixture.writes.slice(failedReceiptWriteCount).map((write) => write.specName),
			['result', 'receipt', 'checkpoint']
		);
		assert.equal(
			DexPlanApplyReceiptSchema.parse(latestWrite(fixture.writes, 'receipt').data).status,
		...
		await executeDexPlanApply({ plan }, fixture.context, dependencies(adapter));
		assert.deepEqual(
			fixture.writes.slice(inconsistentReceiptWriteCount).map((write) => write.specName),
			['result', 'receipt', 'checkpoint']
		);
		assert.equal(
			DexPlanApplyReceiptSchema.parse(latestWrite(fixture.writes, 'receipt').data).status,
...
Deno.test('receipt schema enforces terminal success and failure invariants', async () => {
	...
	const success = DexPlanApplyReceiptSchema.parse(latestWrite(fixture.writes, 'receipt').data);
	assert.equal(
		DexPlanApplyReceiptSchema.safeParse({ ...success, errorCode: 'verification-failed' }).success,
		false
	);
	const failure = {
		...success,
	...
	assert.equal(DexPlanApplyReceiptSchema.safeParse({ ...failure, errorCode: null }).success, false);
	assert.equal(
		DexPlanApplyReceiptSchema.safeParse({ ...failure, resultName: success.resultName }).success,
		false
	);
});
...
Deno.test('succeeded replay rejects inconsistent result mappings before re-emission', async () => {
	...
	assert.deepEqual(fixture.resources.get(checkpointWrite.name), checkpointBeforeReplay);
});
...
Deno.test('success persists result and receipt before the terminal checkpoint', async () => {
	...
	assert.equal(DexPlanApplyCheckpointSchema.parse(fixture.writes.at(-1)?.data).status, 'succeeded');
});
...
Deno.test('missing parent reference fails before the first mutation', async () => {
	...
	const adapter = new FakeDexPlanCommandAdapter();
	await assertPlanFailure(
		executeDexPlanApply({ plan }, fixtureContext().context, dependencies(adapter)),
...
Deno.test(
	...
	async () => {
		...
		assert.equal(failedCheckpoint.taskIdsByClientRef['selected-task'], selected.id);

		adapter.tasks.set('later-duplicate', rawTask({ ...selected, id: 'later-duplicate' }));
		adapter.failCreateCall = null;
		await executeDexPlanApply({ plan }, fixture.context, dependencies(adapter));
...
Deno.test('existing hierarchy detaches every moving task before reparenting', async () => {
	...
	await executeDexPlanApply({ plan }, fixtureContext().context, dependencies(adapter));
...
Deno.test('invalid inverse hierarchy and blocker edges fail before mutation', async () => {
	...
	assert.equal(hierarchyAdapter.createCalls.length, 0);
	assert.equal(
		DexPlanApplyReceiptSchema.parse(latestWrite(hierarchyFixture.writes, 'receipt').data).errorCode,
		'existing-graph-invalid'
	);
...
Deno.test('idempotency conflict rejects changed content under the same plan id', async () => {
	...
	await assertPlanFailure(
		executeDexPlanApply({ plan: changed }, fixture.context, dependencies(adapter)),
		'idempotency-conflict'
	);
	assert.equal(adapter.createCalls.length, 1);
	assert.deepEqual(fixture.resources.get(checkpointWrite.name), checkpointBeforeConflict);
	assert.equal(
		DexPlanApplyReceiptSchema.parse(latestWrite(fixture.writes, 'receipt').data).errorCode,
		'idempotency-conflict'
	);
