...
export const AutomatedVerificationLaneIdSchema = z.enum(['browser', 'check', 'unit', 'structural']);
...
export const VerificationFanoutArgumentsSchema = z.object({
	...
	lanes: z.array(AutomatedVerificationLaneIdSchema).max(4)
});
...
const VerificationFanoutReportContentSchema = z.object({
	...
	results: z.array(VerificationLaneResultSchema).max(4),
	passed: z.boolean()
});
