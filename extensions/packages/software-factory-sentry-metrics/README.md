...
Send bounded Software Factory effectiveness metrics and reconstructed lifecycle traces to Sentry after a work item reaches a terminal state.
...
```text
terminal Factory data
        ↓ deterministic flow report
bounded metrics + reconstructed run/stage trace
        ↓ best-effort flush
Sentry Metrics and Traces + local receipt
```

## Properties

- **Terminal only:** active or unknown runs are rejected.
- **Best effort:** a missing DSN, SDK error, or failed flush writes a receipt instead of failing delivery.
- **Idempotent:** the Factory's terminal journal version becomes a one-way SHA-256 receipt key, preventing duplicate emission.
...
## Emit from terminal Factory data

`emit_flow_report` is the preferred method. It reads the named Factory model's persisted data, rebuilds the canonical flow report, verifies terminal status, and emits the bounded projection.
...
Obtain the source Factory ID from `swamp model get <factory-name> --json`; do not invent or copy an ID from an example. Run emission after the Factory summary or terminal workflow. Observability should remain an `allowFailure` or otherwise non-gating step.
...
Every attempt writes `receipt-<sha256>` with status `emitted`, `duplicate`, `unavailable`, or `failed`. Missing-DSN receipts remain retryable after configuration. Emitted and failed network attempts become final for that terminal key so retrying a workflow cannot double-count an uncertain flush.

Call `verify_flow_receipt` with the same source arguments after emission. It writes `coverage-<sha256>` as `observed`, `degraded`, or `missing` and fails when no emitted or duplicate receipt exists. Put this check after the Factory has entered its terminal state: a Sentry outage becomes a visible observer-workflow failure without changing the Factory outcome.

For automatic coverage, route every terminal outcome through an outcome-specific observability stage. Its workflow should first advance to the durable terminal state, then call `summary`, `emit_flow_report`, and `verify_flow_receipt`. A manual post-run command is recovery tooling, not lifecycle integration.
