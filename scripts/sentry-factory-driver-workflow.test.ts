...
Deno.test("Sentry Factory driver converges machine-authorized repairs through terminal Delivery", async () => {
  ...
  assert.equal(workflow.version, 15);
  ...
  const requiredOrder = [
    ...
    "record-verification-freshness-recovery",
  ...
  assert.match(
    stepByName(workflow, "assert-single-integration-receipt").guard ?? "",
    /stage\.id != "implementation"/,
  );
  ...
  assert.match(
    freshnessInputs,
    /\\"changedPaths\\" in data\.latest.*-verification/,
  );
  assert.match(
    freshnessInputs,
    /\\"workflowRunId\\" in data\.latest.*-verification/,
  );
  assert.doesNotMatch(freshnessInputs, /has\(data\.latest/);
  const recordFreshness = stepByName(
    workflow,
    "record-verification-freshness-recovery",
  );
  assert.equal(recordFreshness.task.methodName, "record_artifact");
  assert.equal(
    recordFreshness.task.inputs?.name,
    "verification-freshness-recovery",
  );
  assert.match(String(recordFreshness.task.inputs?.payload), /change-freshness-recovery/);
  assert.match(
    String(
