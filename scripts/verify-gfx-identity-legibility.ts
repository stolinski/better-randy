// Proves the candidate gfx.computer alpha-cell variants at the sizes they have
// to survive, and captures the review bundle Scott ratifies from.
//
// Three gates, all deterministic:
//   1. Feature width — the tightest ink or counter of a mark, in real pixels at
//      a 16px favicon. Below ~1.5px a feature stops resolving.
//   2. Raster fidelity — each mark is rasterized at 16/24/32/48 and compared to
//      its own 256px reference. Ink coverage must not drift (the form neither
//      fills into a blob nor thins away) and the local luminance range must
//      hold (the form does not mush toward mid-grey).
//   3. Contrast — WCAG ratios for every colour pairing the bundle sanctions,
...
// and a non-zero exit when any gate fails.
...
const SPEC_PLATE = '4K / WebGPU / alpha';
...
interface VariantArtwork {
	readonly variant: GfxAlphaCellVariant;
	readonly markTile: string;
	readonly markMonoDark: string;
	readonly markMonoLight: string;
	readonly logotype: string;
	readonly logotypeMonoDark: string;
	readonly logotypeMonoLight: string;
}

function buildVariantArtwork(variant: GfxAlphaCellVariant): VariantArtwork {
	const { deck, monoDark, monoLight } = GFX_IDENTITY_PALETTES;
	return {
		variant,
		markTile: svgDataUrl(renderGfxIdentityMarkSvg(variant.id, deck)),
		markMonoDark: svgDataUrl(renderGfxIdentityMarkSvg(variant.id, monoDark)),
		markMonoLight: svgDataUrl(renderGfxIdentityMarkSvg(variant.id, monoLight)),
		logotype: svgDataUrl(renderGfxIdentityLogotypeSvg(variant.id, deck)),
		logotypeMonoDark: svgDataUrl(renderGfxIdentityLogotypeSvg(variant.id, monoDark)),
		logotypeMonoLight: svgDataUrl(renderGfxIdentityLogotypeSvg(variant.id, monoLight))
	};
}
...
interface RasterMeasurement {
	...
	/** Separate ink shapes the mark resolves into — corner marks, checker cells, one letter. */
	readonly inkRegions: number;
	/** Counters fully enclosed by ink, such as the G's pocket. */
 ...
 * ink and the plate, so every variant is measured on its own alternation rather
 * than on its silhouette.
 */
const INK_THRESHOLD =
	(relativeLuminance(GFX_IDENTITY_PALETTES.deck.ink) +
		relativeLuminance(GFX_IDENTITY_PALETTES.deck.inkAlternate!)) /
	2;
...
async function rasterizeMark(page: Page, markUrl: string, size: number): Promise<Buffer> {
	await page.setViewportSize({ width: Math.max(size, 64), height: Math.max(size, 64) });
	await page.setContent(
		`<!doctype html><style>html,body{margin:0;background:${DECK}}img{display:block;width:${size}px;height:${size}px}</style><img src="${markUrl}" alt="">`
	);
...
.topbar{align-items:center;background:${PANEL};border-block-end:1px solid ${LINE};display:flex;gap:20px;min-block-size:56px;padding-inline:20px 16px}
...
async function captureSheet(
	...
	const bytes = await page.screenshot({
		clip: { x: 0, y: 0, width, height, scale: deviceScale }
	});
...
function topbarMarkup(artwork: VariantArtwork): string {
...
function mastheadMarkup(artwork: VariantArtwork): string {
	...
	<div class="row" style="gap:28px">
		...
		<div class="stack" style="gap:14px">
			...
			<p class="address" style="font-size:13px">gfx.computer</p>
		</div>
	</div>
	<div style="background:${LINE};block-size:1px;margin-block:40px 20px"></div>
	<p class="plate" style="font-size:11px">${SPEC_PLATE}</p>
</section>`;
}

function socialCardMarkup(artwork: VariantArtwork): string {
	...
	<div class="stack" style="gap:24px">
		...
		<p class="address" style="font-size:20px">gfx.computer</p>
	</div>
	<div style="align-items:center;display:flex;gap:14px;inline-size:100%">
		<div style="background:${LINE};block-size:1px;flex:1"></div>
		<p class="plate" style="font-size:14px">${SPEC_PLATE}</p>
	</div>
</section>`;
}

function monochromeMarkup(artwork: VariantArtwork): string {
...
/** Magnification of the true small raster, so a merged counter is visible rather than inferred. */
const FAVICON_MAGNIFICATION = 6;

function faviconMarkup(
	rasters: readonly { readonly size: number; readonly url: string }[]
): string {
...
/** The four levers that separate the variants, read off the variant itself. */
function variantSpecLine(variant: GfxAlphaCellVariant): string {
	const field =
		variant.mark.kind === 'floating-checker'
			? `${variant.mark.cellCount}×${variant.mark.cellCount} floating`
			: `${variant.mark.cellUnits}u bleed${variant.mark.letter ? ` + ${variant.mark.letter}` : ''}`;
	const lean = variant.shearDegrees === 0 ? 'upright' : `${Math.abs(variant.shearDegrees)}° lean`;
	return [
		field,
		lean,
		`gutter ${variant.logotypeCellGutter}`,
		`track ${variant.logotypeTrackingUnits}`
	].join(' / ');
}

/** Width of one comparison column, and the gap between columns. */
const COMPARISON_COLUMN_WIDTH = 288;
const COMPARISON_GAP = 40;
const COMPARISON_PADDING = 48;

function variantComparisonMarkup(artworks: readonly VariantArtwork[]): string {
	const column = (artwork: VariantArtwork): string =>
		`<div class="stack" style="gap:20px;inline-size:${COMPARISON_COLUMN_WIDTH}px">
			<img src="${artwork.markTile}" alt="" style="block-size:176px;inline-size:176px">
			<img src="${artwork.logotype}" alt="GFX" style="block-size:44px;align-self:flex-start">
			<div class="stack" style="gap:8px">
				<p class="caption">${artwork.variant.title} — ${artwork.variant.id}</p>
				<p class="plate">${variantSpecLine(artwork.variant)}</p>
			</div>
			<p style="color:${MUTED};font-size:13px;line-height:1.5;margin:0">${artwork.variant.concept}</p>
		</div>`;
	return `<div class="row" style="align-items:flex-start;gap:${COMPARISON_GAP}px;padding:${COMPARISON_PADDING}px">${artworks.map(column).join('')}</div>`;
}

const COMPARISON_SHEET_WIDTH =
	COMPARISON_COLUMN_WIDTH * GFX_ALPHA_CELL_VARIANTS.length +
	COMPARISON_GAP * (GFX_ALPHA_CELL_VARIANTS.length - 1) +
	COMPARISON_PADDING * 2;
...
/**
 * Every variant draws from the same achromatic set, so the pairings are the same
 * for all four: ink against the plate, the second checker neutral against the
 * plate, and both one-ink cuts on their worst-case backgrounds.
 */
...
function buildReport(verifications: readonly VariantVerification[]): string {
	const lines: string[] = [
	...
	for (const verification of verifications) {
		const { variant } = verification;
		lines.push(
			`## ${variant.title} (\`${variant.id}\`)`,
			'',
			variant.concept,
			'',
			`Levers: ${variantSpecLine(variant)}.`,
			'',
			`Tightest cell at 16px: **${verification.featurePixelsAt16.toFixed(2)}px**.`,
			`Reference (${REFERENCE_SIZE}px): coverage ${verification.reference.inkCoverage.toFixed(3)},`,
			`${verification.reference.inkRegions} ink region(s), ${verification.reference.enclosedCounters} enclosed counter(s).`,
			'',
			'| Rendered size | Ink coverage | Coverage drift | Ink regions | Enclosed counters |',
			'| --- | --- | --- | --- | --- |'
		);
		for (const raster of verification.rasters) {
			const drift = Math.abs(raster.inkCoverage - verification.reference.inkCoverage);
			lines.push(
		...
		for (const check of verification.contrast) {
			lines.push(
				`| ${check.pairing} | ${formatRatio(check.ratio)} | ${formatRatio(check.minimum)} |`
			);
		}
		lines.push(
			'',
			verification.failures.length === 0
				? '**Passes every gate.**'
				: `**Fails:** ${verification.failures.join('; ')}.`,
			''
		);
	}
	return `${lines.join('\n')}`;
...
const verifications: VariantVerification[] = [];
const artworks: VariantArtwork[] = [];

try {
	for (const variant of GFX_ALPHA_CELL_VARIANTS) {
		const artwork = buildVariantArtwork(variant);

		const referenceBytes = await rasterizeMark(page, artwork.markTile, REFERENCE_SIZE);
		const reference: RasterMeasurement = {
			size: REFERENCE_SIZE,
			...measureRaster(referenceBytes, INK_THRESHOLD)
		};

		const rasters: RasterMeasurement[] = [];
		const previews: { size: number; url: string }[] = [];
		for (const size of FAVICON_SIZES) {
			const bytes = await rasterizeMark(page, artwork.markTile, size);
			rasters.push({ size, ...measureRaster(bytes, INK_THRESHOLD) });
			previews.push({ size, url: pngDataUrl(bytes) });
		}

		const featurePixelsAt16 = measureGfxIdentityFeaturePixels(variant.id, 16);
		...
		await captureSheet(
			...
			`favicon-${variant.id}.png`,
			2
		);
		await captureSheet(page, topbarMarkup(artwork), 1280, 57, `topbar-${variant.id}.png`, 2);
		await captureSheet(page, mastheadMarkup(artwork), 1280, 310, `masthead-${variant.id}.png`, 2);
		await captureSheet(
			page,
			monochromeMarkup(artwork),
			1000,
			190,
			`monochrome-${variant.id}.png`,
			2
		);
		// The social card is the shipped asset, so it is captured at its exact
		// 1200x630 pixel size rather than a review scale.
		await captureSheet(page, socialCardMarkup(artwork), 1200, 630, `social-${variant.id}.png`, 1);

		verifications.push({ variant, featurePixelsAt16, reference, rasters, contrast, failures });
	}

	await captureSheet(
		page,
		variantComparisonMarkup(artworks),
		COMPARISON_SHEET_WIDTH,
		540,
		'alpha-cell-variants.png',
		2
	);
} finally {
	await browser.close();
...
// Captures are generated output keyed by variant id, so anything left over from
// a retired candidate is stale and must not stay in the review bundle.
...
writeFileSync(reportPath, buildReport(verifications), 'utf8');

const failed = verifications.filter((verification) => verification.failures.length > 0);
for (const verification of verifications) {
	process.stdout.write(
		`${verification.variant.id}: ${verification.failures.length === 0 ? 'pass' : `FAIL — ${verification.failures.join('; ')}`}\n`
	);
}
