...
await sleep(2200);
for (let i = 0; i < 40; i++) {
	const ok = await ev(`!!(window.__hivizTimeline && document.querySelector('canvas'))`);
	if (ok) break;
	await sleep(300);
}
await ev(`window.__hivizDofPreviewPlane = undefined; window.__hivizTimeline.seekProgress(0.5);`);
await sleep(800);
