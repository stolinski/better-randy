...
Routine rendered-safety verification runs `pnpm verify:layout-contract`. One isolated headless Chrome process reuses Preset pages and emits strict numeric **Layout Contract Frames** across every deliverable Preset, Pack, orientation, and critical frame. It records native target size, font readiness, semantic readable-identity coverage, title/platform-safe intersections, clipping, cap-height floors, reading windows, canonical/replay geometry digests, and stable-layout deltas. A successful run writes JSON only: no PNG, data URL, screenshot path, visible browser window, or evidence archive. Missing identities and unavailable timing fail closed. The full matrix has a twelve-minute wall-time bound, measured from loaded multi-agent operation rather than an idle-machine estimate.

The `layout-contract` Factory lane owns this receipt for rendered source. Verification runs ordinary CPU-heavy lanes concurrently, then runs the GPU/browser Layout Contract lane in isolation so internal test contention cannot consume its twelve-minute matrix budget. One exact twelve-minute timeout or transient runtime-readiness miss receives one bounded operational retry; a repeated failure remains explicit with its subprocess diagnostics. Engine and Pipeline changes do not acquire a subjective aesthetic claim; directly authored Preset/Pack appearance still pauses for separate aesthetic evidence. Pixel-only questions such as antialiasing, blur, banding, codec artifacts, and composited local contrast use an explicitly requested bounded **Pixel diagnostic**, never an automatic corpus capture.
