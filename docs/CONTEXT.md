...
- **Pack system is partially wired.** **Color and font** Roles reach pixels via `resolveAppearanceVars` (CSS custom properties injected on the mounts — the live path). The first **structural** Role, `depth`, now reaches pixels too, via the typed `resolveDepthTreatment` (specific→core fallback) — proven on the `newspaper` card's hard-offset shadow. The other structural Roles (`edge` / `light` / `material`) are still declared + boot-validated but inert; the old unused `resolveStyle` / `resolveRole` accessors have been removed. The committed direction is to **finish** it (wire the remaining structural resolution to pixels), not cut it — see [`engine-architecture.md`](engine-architecture.md) § Appearance and [`roadmap.md`](roadmap.md). Until then, treat the glossary's Pack/Role definitions as the *target* model for `edge`/`light`/`material`, and the live path as current reality for color, font, and depth.
