...
A swappable **palette** that resolves **sound-event** Roles to concrete audio samples. A palette is exactly its sounds — an event it doesn't carry resolves to **silence** (no core fallback; amended 2026-07-02 after by-ear testing showed an inaudible fallback made palette swaps read as a dead control). `core` is an ordinary palette that covers every event. Assigned **per Layer** (in the Layer's inspector), **not** per composition — there is no whole-piece sound pack; a Layer with no kit is silent. "Choosing a sound style" re-sounds *that Layer*. Carries **sound only** — no appearance (that is the Pack), no motion-timing (that is intrinsic to the motion). Cascade: **Layer** (which kit) → **motion / event** (override one sample). Unlike the appearance **Pack** (one global dress, [ADR-0023](adr/0023-pack-is-appearance-only.md)), the kit lives on the Layer because sound is event-driven and granular.
