...
- **Pipeline registries and identity registry** — register both new variants and their Identity Specs. Add the required committed `static/surface-posters/<type>.webp` fallback for the new Surface; composition-specific posters remain capture-on-view cache entries.
