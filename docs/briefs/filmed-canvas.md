...
| --------- | --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Rest pose | 0.00            | Camera at `pose`: yaw −19°, pitch −9° (below the page and to its left, looking up), roll 0, distance 0.6, aim `(0.5, 0.52)` — the middle row's middle thumbnail (`pageAnchor` `(0.54, 0.5)` centres the grid in the plane). Focus follows the aim; aperture 1, band 0. The page already fills the frame. |
| Push      | 0.00 → 1.00     | One `travel` for the whole cut, ease `smooth`: distance 0.6 → 0.42 with a slow arc, yaw −19° → −12° and pitch −9° → −6°, aim fixed. A lone flat plane gives a straight dolly nothing to parallax against, so it reads as a zoom; the arc rotates the page's perspective around the aimed thumbnail while the lens shallows with the approach (the far-row blur roughly doubles by the end), which is what a camera does and a zoom cannot. |
