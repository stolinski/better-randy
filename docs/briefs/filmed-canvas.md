...
| --------- | --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Rest pose | 0.00            | Camera at `pose`: yaw −16°, pitch −8° (below the page, looking up), roll 0, distance 0.5, aim `(0.5, 0.52)` — the middle row's middle thumbnail (`pageAnchor` `(0.54, 0.5)` centres the grid in the plane). Focus follows the aim; aperture 1, band 0. The page already fills the frame. |
| Push      | 0.00 → 1.00     | One `travel` that changes only `distance`, 0.5 → 0.44, ease `smooth`: a slow push for the whole cut, aim and angle fixed. Focus rides the aim, so the sharp band stays on the same thumbnail as the page grows.                                                     |
