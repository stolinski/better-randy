...
- [~] **3. Dead motion controls (S).**
  ...
  - [ ] `entry.enter.ease` is voided for text anims (generic-stagger uses the catalog `spec.enter.easing`; `Workspace.svelte:722-725`). Decision needed: wire per-entry override vs delete the field (ease is arguably intrinsic to the catalog effect). Not done blind.
  - [ ] `cameraMotion` declared, zero consumers. Decision needed: wire a consumer (drive paper card transform from focal-slot center + globalProgress) vs delete. Not done blind.
- [ ] **4. First real ACCEPT (M).** Run Brief→Producer→Critic end-to-end on the **cinematic lower-third** with real Syntax guest copy → first Critic `ACCEPT` in the repo's history; the reference the other 21 are rewritten against. Needs steps 1–3 first.
