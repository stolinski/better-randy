...
2. **Real page furniture, real page type.** `dateLabel` and `source` print on the folio line over the heavy rule; `kicker` is an ink section label; `title` is the grotesque headline (Inter 700, −0.035 em, sized from the visible measure to fill two lines); `author` and `affiliation` form the byline; `body` flows through justified Old Standard TT columns with `column-rule`s, first-line indents, and no paragraph spacing. The sheet is `#dadada`, the ink `#1b1b1b` (`newsprint-substrate.ts`), read off the plates. The headline takes the highlighter: the title accepts the body's bracket-tag mark syntax, gated by a new `titleMarks` flag on the Surface definition — `listSurfaceMarkInstances` enumerates a title's marks before the body's (document order, so `marks.timings[]` stays aligned with the DOM spans the marks renderer walks), the readable contract projects the plain title, and the rubric refuses mark syntax in the title of any Surface that prints it plain.

3. **Motion is a camera, not a card — and the shot is a cut.** The whole piece carries a 2 % push with a hint of drift, a pure function of timeline progress. There is no enter or exit sugar: the definition declares `enterExit: false`, so the controls stay hidden rather than inert. (A landing settle on enter shipped first and was cut the same day — Scott: too abrupt; the subtle push alone is right.) The push is a **texture resample in the physics pass**, never a CSS transform on the DOM: the page rasterizes once and every mark rect holds still, and the halftone, mottling, and scan grain are computed in the resampled page space so the print texture travels with the page instead of crawling over it. The first build animated the DOM and read as jitter on the headline — Scott, same day — for three compounding reasons: the highlighter seeded its chisel edges and pigment from the fragment's screen position, so each frame's sub-pixel shift re-rolled the marker (fixed at the source: marks now seed from their identity, index and fragment, for every Surface); the screen-fixed dot screen and grain crawled across the moving glyphs; and a scaling text layer re-rasterizes. Measured on consecutive frames, the highlighted-headline region's changed pixels fell from 12.5 % to 1.3 %.
...
- **Headline marks as a universal title feature** (rejected: marks are drawn by the paper compositor, so a title mark only means something on a Surface that runs through it; enumerating title marks for every Surface would desynchronise `marks.timings[]` from the DOM on the ones that print their title plain. Hence the per-definition `titleMarks` opt-in and the rubric guard).

## Consequences

- `title-card-newspaper` is re-authored as the page (4 s, body copy, a highlighter on the headline, no tape, no grain Effect); `newspaper-body-test` becomes the body-plate fixture (multi-line highlighter across a quotation). Neither carries enter/exit. `depth-stage-demo` and `depth-of-field-tabletop` move to the `paper` Surface — they prove the depth stage and DOF, not the newspaper. `server-renders-again` (the editorial-mono clipping collage and the pack-diff lock's newspaper representative) is deleted: its rationale was the chip and the `depth: 'none'` claim, both gone.
