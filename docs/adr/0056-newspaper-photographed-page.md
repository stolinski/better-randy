...
3. **Motion is a camera, not a card — and the shot is a cut.** The whole piece carries a 2 % push with a hint of drift, a pure function of `globalProgress`. There is no enter or exit sugar: the definition declares `enterExit: false`, so the controls stay hidden rather than inert. (A landing settle on enter shipped first and was cut the same day — Scott: too abrupt; the subtle push alone is right.)
