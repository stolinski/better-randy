...
| [0034](0034-gui-design-authoring-interface.md) | **Designed, not built** (interaction model; visual design pending) | GUI design — the authoring interface. Three zones (canvas · timeline-outline · inspector); **the timeline IS the layer outline**; **hybrid** manipulation (inspector complete, canvas does position + scale, two-way); selection = row → Layer inspector, click-off → root inspector (transport / Pack / Effects); per-type inspectors + progressive disclosure; per-Layer Sound-kit picker. The **visual design** (look / organization) is a separate pass. Tracked in dex epic `jz2yykvi` |
