...
**Canon (v1 and listed expansions built). Refines [ADR-0027](0027-dof-v1-multiplane-bokeh.md) and [ADR-0021](0021-z-plane-semantics.md).** Stage camera/focus, overlay-at-depth, real scene lighting/shadow, and half-resolution DOF are built. Stage preview/export now share `renderCompositionFrameTo(request)` in `composition-frame-renderer.ts`, with `Workspace.svelte` supplying live dependencies. `resolveMaterialTreatment` also has a typed pixel consumer through the shared alpha-masked CRT scanline ShaderPass; it is not a stage-native material-lighting system. The blockquote below is the historical implementation ledger.

> **Historical implementation ledger — v1 landing.** Validated in an isolated POC (`src/routes/poc/dof3d/+page.svelte`), then **integrated into the engine and Critic-accepted** (2026-06): `state.stage` schema, the `DepthStage` renderer (`src/lib/platform/pipelines/depth-stage.ts`), the then-current `renderAt` branch (preview + export), orientation reflow, and a Critic-ACCEPTed demo (`depth-stage-demo`). ADR-0027's multiplane bokeh stays **Canon and the default** for flat, text-critical pieces. This ADR realizes the _continuous-depth_ mechanism 0027 deferred as a **real 3D scene render** (DOM-as-texture on perspective geometry, per-pixel depth from the geometry, a real lens DOF) — not 0027's "z-map DOM" upgrade. ADR-0021's Z _semantics_ (focal-distance scalar in [0,1], per-Layer defaults) are reused as plane placement.
>
...
>
> **Shared frame seam (amended 2026-07-23).** Stage preview and export no longer carry parallel dispatch code. Both enter `renderCompositionFrameTo(request)` in `composition-frame-renderer.ts`; `Workspace.svelte` supplies the live dependencies. The frame renderer builds complete Surface inputs, queue-orders one live DOM upload, and gives the dimensional stage precedence over DOF and flat compositing. Output classification treats any declared stage as opaque even without `backgroundFill`.
>
...
"stage": {                      // OPTIONAL. Absent ⇒ flat multiplane path (ADR-0027), unchanged.
  ...
  // Scene light is resolved from the Pack's light-treatment Role. Material
  // treatment is consumed by the shared alpha-masked CRT scanline ShaderPass.
}
```
...
- **Structural Pack Roles** — `light-treatment` now drives real stage lighting/shadow. `material-treatment` has a separate typed pixel consumer through the shared alpha-masked CRT scanline ShaderPass; it is not a stage-native material system.
- **Unification door left open** — the hybrid (flat default + stage for depth) is what's validated, but the stage reuses the shared capture + post path so it does not preclude later collapsing the two paths into one (flat = degenerate stage). The scariest blocker to that, text fidelity, is **validated**: a fronto-parallel plane through the stage is as crisp as a 1:1 blit at native density (POC `mode=ref` vs `mode=flat`; captures `docs/critic-captures/poc/unify-*`). Full unification is not current direction; the historical exploration lives at [`../history/unified-webgpu-compositor.md`](../history/unified-webgpu-compositor.md).

The stage selector/schema, scene renderer, per-pixel depth, mip-gather DOF, `stage.camera`/focus controls, orientation framing, and proving Presets are shipped. The retained POC (`src/routes/poc/dof3d/`) is historical validation evidence; current implementation authority lives in `composition-frame-renderer.ts`, `DepthStage`, and [`../engine-architecture.md`](../engine-architecture.md).
