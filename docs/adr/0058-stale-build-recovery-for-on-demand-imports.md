...
4. **A `load` whose `__data.json` never reached the origin recovers the same way** (added 2026-09-04 for GFX-COMPUTER-12). Layer 2 only fires once the poll has seen the new build; inside the polling window, and across the restart itself, a client-side navigation still runs and SvelteKit fetches `__data.json` itself. That fetch is not an import, so nothing above covered it: the tab got a dead error page and Sentry got an unhandled `Failed to fetch`. `hooks.client.ts` now owns `handleError` rather than handing it straight to Sentry, and a failure `isOriginFetchFailure` classifies takes `reloadIfBuildIsStale()` and is reported only when the tab is not reloading. `isOriginFetchFailure` matches the whole message, because a browser that never reached the origin has no detail to add; anything with detail in it (a dynamic import's URL) belongs to `isModuleLoadFailure`.

   On a **development** host that failure is answered without the recovery and without Sentry, because the recovery cannot run there: SvelteKit hard-wires `updated.check()` to `false` under `__SVELTEKIT_DEV__`, so a tab can never learn that the dev server restarted. Vite's own client already reloads the page when its socket reconnects, and the only thing that produces this message in dev is a dev server that went away — which is where every event on GFX-COMPUTER-12 came from.
...
- An origin that is genuinely unreachable — down, or the network gone — still reports. `updated.check()` has to reach the same origin, so it cannot see a newer build either, `reloadIfBuildIsStale()` resolves false, and the error reaches Sentry exactly as before. Only the rebuild case goes unreported, and there the rebuild was the failure.
