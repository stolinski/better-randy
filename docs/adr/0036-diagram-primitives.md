...
Elements carry `id`, `type`, explicit position, content, and standard timing (`enter`/`exit` sugar, **Cascade** anchor, ADR-0035 keyframe channels where they apply). Edges reference node ids. Exact schema field placement (a `diagram` group on the Surface vs a top-level `blocks[]`) is decided in the schema task with the byte-identical round-trip gate in force.
