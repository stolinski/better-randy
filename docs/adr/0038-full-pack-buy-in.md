...
**Canon (built).** Optional typography color overrides, Pack-core fallback, registry-visible immunity, and Pack-swap validation are live. `PACK_IMMUNE_PIPELINE_KEYS` is the complete current authority; examples in the decision-time prose are not a complete immunity list.
