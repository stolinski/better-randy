...
- _Extended 2026-08-04 (dex `e2z4whnu`):_ `field-ink-treatment` is the optional foreground companion to `field-treatment`. Direct-on-field Diagram ink, plain-Surface body ink, and plate-less full-frame Overlay ink resolve authored composition ink → field ink → mandatory `ink-treatment`; card/document ink and transparent-over-footage Pipeline ink keep their existing paths. The claim is explicit Pack data, never a runtime luminance flip. Optionality is safe because a Pack whose ordinary ink already contrasts with its field needs no duplicate value; if supplied, the authoring validator requires a real colour.
