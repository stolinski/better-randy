...
**Limits are ratified here and enforced later.** `PUBLIC_EXPORT_RUNTIME_LIMITS` fixes the public envelope: 15 seconds, 60 fps, 900 frames, 64 MiB per frame, 8 MiB of audio, a 2 GiB output ceiling, 2 concurrent sessions, a 15-minute idle expiry inside a 30-minute hard lifetime, and 8 GiB of free temp disk before the host admits traffic. The output ceiling is derived from the measured per-frame cost above with headroom (`RATIFIED_NATIVE_OUTPUT_BYTES_PER_FRAME`), and `public-runtime-contract.test.ts` keeps the numbers mutually consistent. Request-time rejection of over-limit sessions is a separate change; today the export session store consumes the deployment inputs and the idle expiry.
