...
- 🔨 **Finish the structural Pack → pixel contract.** Color + font Roles reach pixels via `resolveAppearanceVars`; the structural Roles (`edge` / `depth` / `light` / `material`) are declared + boot-validated but color-filtered out and inert, and `resolveStyle`/`resolveRole` are dead. Wire structural resolution to pixels; delete the dead path. Multi-pack is north-star-core, so finish — defer only if a clean staging emerges. (ADR-0014/0019/0023/0024 give the model.)
