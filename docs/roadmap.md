...
5. 🔨 **Sentry-driven self-healing repair loop** — epic `ueo65fsy`: ingest bounded unresolved-issue snapshots, reconcile current failures against historical noise, and route verified repair candidates through the existing human-gated Planning and Delivery Factories. The first slice (`lssrzdm9`) is read-only and cannot mutate Sentry, Dex, or source code.
6. **Finish Pack contract standardization** — task group `oa9tr4wy`: close the role-contract registry, then add catalog-ratification evidence. This makes future Pack work factory-safe.
