...
6. ✅ **Sentry-driven self-healing repair loop** — shipped 2026-08-18 (epic `ueo65fsy`): immutable unresolved-issue snapshots are reconciled against release freshness and official Dex state, then one stable issue at a time is routed through the existing human-gated Planning Factory and Dex Plan Applier. Ambiguous, stale, duplicate, unconfirmed, active-WIP, and incomplete paths fail closed.
7. 🔨 **Finish Pack contract standardization** — task group `oa9tr4wy`: the closed role-contract registry shipped in `hqlyu0rb`; task `ie65019q` is adding machine-checkable catalog-ratification evidence. This makes future Pack work factory-safe.
8. **Build the broad Starter-template library** — task `dghxldci`: audit creator-job coverage, then author the missing categories and coordinated episode suite against the completed Pack/quality gates.
9. **Pull production and research work by demand** — Resolve follow-ups, rubric research, Critic research, optional diagram polish, and rendering residuals remain valid but do not interrupt the ordered product runway.
