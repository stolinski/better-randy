...
10. 🔨 **Launch gfx.computer as a public WebMCP authoring experience** — active (epic `gi41tjku`): rename the current product and technical vocabulary to GFX with explicit Legacy Supers readers, ratify and install a new logo without redesigning the editor, support deterministic rendering in standard WebMCP-enabled browsers, expose complete shared GUI/WebMCP create-to-export operations, provide no-account browser sessions and bounded public export, deploy `gfx.computer`, and close on exact bidirectional parity plus release evidence. Dex carries the 40-leaf ordered implementation graph and its unique ready leaf. The browser/WebMCP capability lane is measured ([`standard-browser-rendering-probe.md`](standard-browser-rendering-probe.md)) and the public host is ratified from live probes as a Node/ffmpeg origin with Cloudflare limited to DNS and proxy ([ADR-0052](adr/0052-public-runtime-and-retention-architecture.md)).
11. **Pull production and research work by demand** — Resolve follow-ups, rubric research, Critic research, optional diagram polish, and rendering residuals remain valid but do not interrupt the ordered product runway.
...
- 📐 **Dimensional Stage expansion (initiative: 3D Canvas Upgrade)** — [ADR-0051](adr/0051-pipeline-defined-dimensional-stage-geometry.md) evolves the existing `depth` Stage in place from its fixed captured planes into a TypeGPU-native depth-tested scene compositor. Registered Surface, Block, and Overlay Pipelines may contribute bounded geometry while retaining their existing Layer identity; there is no generic scene tree, sixth Layer, arbitrary model-import requirement, or parallel scene Stage. Procedural geometry, instancing, compute deformation, SDF/raymarching, materials, cameras, lighting, and shadows expand only through concrete Pipeline claims. Three.js remains demand-gated behind proof that an approved Pipeline cannot meet its Identity claim natively.
