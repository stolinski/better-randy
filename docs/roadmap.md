...
- 🧭 **Opaque-piece probe mode** (rubric-gap from the depth-stage Critic, 2026-06) — **luma auto-detection shipped** (`_probe-channel.ts`): `probe-banding`, `probe-edge-aa`, and `probe-ink-coverage` now resolve their measurement channel — auto-detecting opaque→luma (or forced with `--channel luma`), `probe-ink-coverage` measuring deviation from the field's modal luma — so R3/R4/R5/Q9 ARE measurable on opaque full-frame pieces, not only alpha-keyed overlays. Residual under `9w7kdptf`: alpha-preserving verification of the opaque *export* rides the export-decode lane (`probe-export-decode.ts`), and any remaining per-probe `--opaque` ergonomics.
