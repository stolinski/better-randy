...
The active `Workspace` is the shipped three-zone interface from ADR-0034: canvas center with direct position/scale manipulation and view controls, `TimelineOutline` across the bottom, and selection-driven `Inspector` on the right. It autosaves through ADR-0032's fork model. Sound is edited per cue/motion in the inspector and Sound rail; the proposed per-Layer Sound-kit picker was removed.
