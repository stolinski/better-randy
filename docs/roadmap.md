...
9. 🔨 **Launch gfx.computer as a public WebMCP authoring experience** — active (epic `gi41tjku`): rename the current product and technical vocabulary to GFX with explicit Legacy Supers readers, ratify and install a new logo without redesigning the editor, support deterministic rendering in standard WebMCP-enabled browsers, expose complete shared GUI/WebMCP create-to-export operations, provide no-account browser sessions and bounded public export, deploy `gfx.computer`, and close on exact bidirectional parity plus release evidence. Dex carries the 40-leaf ordered implementation graph; the unique ready leaf is the browser/WebMCP capability probe (`n7txt7sq`).
10. **Pull production and research work by demand** — Resolve follow-ups, rubric research, Critic research, optional diagram polish, and rendering residuals remain valid but do not interrupt the ordered product runway.
