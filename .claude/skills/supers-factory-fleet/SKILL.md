...
- Unrelated parent-checkout dirt never blocks fanout. Each implementation starts from the exact committed parent `HEAD` in its own linked worktree; only overlap or drift on that task's validated paths may pause it.
