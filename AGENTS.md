...
- **Delivery runs through the lean factory.** `gfx-factory` (a `@swamp/software-factory` instance) works Dex tasks in isolated worktrees: implement → deterministic verify → human aesthetic review only for visual changes → cherry-pick onto main. Drive it with the `gfx-factory` skill (which builds on the pulled generic `software-factory` skill). The `sentry-autofix` workflow is the automated Sentry repair lane. Keep both definitions small — behavior belongs in the stage YAML and prompts, not in new extension code.
...
- **Develop local-only; the public target is a Node origin.** Work runs on the local dev server. The public gfx.computer demo is one Node/ffmpeg origin with Cloudflare supplying DNS and proxy only ([ADR-0052](docs/adr/0052-public-runtime-and-retention-architecture.md)) — never treat Workers/edge constraints as binding, never pick libraries or architectures "for Cloudflare compatibility," and never reach for Cloudflare services (KV, D1, R2, Workers). Server routes freely use Node APIs, the local filesystem, and local binaries.
...
| Working Dex tasks/epics through the Delivery factory                               | [`.claude/skills/gfx-factory/SKILL.md`](.claude/skills/gfx-factory/SKILL.md) — the `gfx-factory` instance, isolated worktrees, deterministic verify, serialized integration                                                                                                         |
