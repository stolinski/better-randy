...
- **Every deliverable Preset is orientation- and Pack-neutral.** One Preset must render and look good in _both_ horizontal and vertical _and_ under every Pack ([ADR-0039](docs/adr/0039-pack-neutral-compositions-and-listing-hygiene.md)). Never create per-orientation or per-Pack variants of a composition, and never ask whether a preset "should support" vertical or horizontal — _how_ it reflows is designable; _whether_ it reflows is not. Existing fixture-only recompositions and Pack calibration re-dresses document engine gaps or validate Packs; they are not listed deliverables or authoring precedent.
...
- **Proof-corpus discipline (build-harness — not product law).** The Critic is an optional adversarial observation tool (see [`docs/critic.md`](docs/critic.md)); its prose is advisory and has no Delivery transition, mutation, approval, rejection, or routing authority. Deterministic closed-code checks supply objective routing evidence. Exact-evidence-bound human aesthetic approval is the only subjective acceptance authority.
- **Supers Delivery profile override.** Keep the pulled generic `.claude/skills/software-factory` skill and upstream extension registry unchanged. For Supers Delivery, use `.claude/skills/supers-factory-fleet`: record the project `change-summary`, run the deterministic verification bundle and exact-evidence human gate, and return objective incompleteness to implementation before completion-only reconciliation.
...
| Verifying a Preset's render                                                        | [`docs/project-control-plane.md`](docs/project-control-plane.md) (deterministic matrix), [`docs/quality-rubric.md`](docs/quality-rubric.md), [`docs/animation-rubric.md`](docs/animation-rubric.md); [`docs/critic.md`](docs/critic.md) only for optional human-requested advisory follow-up |
...
| Driving multiple approved Factory epics concurrently                               | [`.claude/skills/supers-factory-fleet/SKILL.md`](.claude/skills/supers-factory-fleet/SKILL.md) — Swamp work-item state + Pi-managed isolated worktrees + serialized integration                                                                                                              |
| Auditing repo policy or planning-state drift (Swamp audits, policy-sweep, CI gate) | [`docs/project-control-plane.md`](docs/project-control-plane.md) — the repo-audit model, the planning drift classes, and the remediation loop                                                                                                                                                |
