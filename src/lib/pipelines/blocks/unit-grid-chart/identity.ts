...
export const unitGridChartIdentity: IdentitySpec = {
	kind: 'graphic',
	claim:
		'an editorial normalized parts-of-whole grid whose geometry remains faithful to its validated inline data',
	dimensions: [
		{
			name: 'mark-treatment',
			viaPack: 'chart.mark',
		...
		},
		{
			...
			implementation:
				'src/lib/pipelines/blocks/unit-grid-chart/CanvasSource.svelte + src/lib/pipelines/shader-passes/chart-mark-renderer.ts — chart chrome is flat DOM ink and data marks are analytic masks; enclosing Surface/stage treatments own any edge, depth, or light.',
			definition:
		...
		},
		{
			...
			definition: 'Fail-closed factual contract for the normalized parts-of-whole grid.',
			probe: {
				kind: 'script',
