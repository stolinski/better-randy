...
export const lineChartIdentity: IdentitySpec = {
	kind: 'graphic',
	claim:
		'an editorial line trend with connected points whose geometry remains faithful to its validated inline data',
	dimensions: [
		{
			name: 'mark-treatment',
			viaPack: 'chart.mark',
		...
		},
		{
			...
			implementation:
				'src/lib/pipelines/blocks/line-chart/CanvasSource.svelte + src/lib/pipelines/shader-passes/chart-mark-renderer.ts — chart chrome is flat DOM ink and data marks are analytic masks; enclosing Surface/stage treatments own any edge, depth, or light.',
			definition:
		...
		},
		{
			...
			definition:
				'Pack-driven connected lines and premultiplied data-point marks remain local to factual renderer geometry and never texture chart chrome.',
			probe: {
				kind: 'script',
		...
		},
		{
			...
			definition: 'Fail-closed factual contract for the connected line trend.',
			probe: {
				kind: 'script',
