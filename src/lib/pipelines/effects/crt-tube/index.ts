...
import {
	crtTubeEffectDefinition,
	type CrtTubeParams as CrtTubeParamsDefinition
} from './definition';

export type CrtTubeParams = CrtTubeParamsDefinition;
...
export const crtTubeEffectRenderer: EffectRenderer<CrtTubeParams> = {
	...
	pass: {
		...
		pack: (params, ctx) => ({
			...
			maskMode:
				(params.mask ?? 'slot') === 'grille' ? 2 : (params.mask ?? 'slot') === 'shadow' ? 1 : 0,
