...
project so the GFX text-animation engine can ship offline-deterministic
...
`depth-parallax-words`) keep `showcase: null`; GFX dispatches those
...
governs reuse of these files. The GFX repository ships the vendored
