<script lang="ts">
	...
	import type { AnnotationBody, AnnotationMarkColors } from '$lib/annotations/annotation-marks';
	import {
		getEditorSelection,
		renderEditorBody,
		serializeEditorBody,
	...
	interface Props {
		...
		prepareMarkStyle?: (style: AnnotationMarkStyle) => Promise<void>;
	}
	...
	const markOperationGuard = new AsyncAuthoringOperationGuard();
	onDestroy(() => markOperationGuard.dispose());
	...
	async function applyMark(style: AnnotationMarkStyle): Promise<void> {
		if (!editor) {
			return;
		}
		...
		const generation = markOperationGuard.begin();
		const previousBody = body;
		try {
			await prepareMarkStyle?.(style);
		} catch (cause) {
			console.error('Failed to load Annotation renderer.', { style, cause });
			return;
		}
		if (!markOperationGuard.isCurrent(generation) || !editor || body !== previousBody) return;
